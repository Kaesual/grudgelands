...
through `(p)`/`(q)`. The combat marker inventory was 32; the fresh-server
...
filter), the mob-pressure round (2026-09-16) adds the 41st to 43rd, the
attack-cadence patch of the section below, round 5 R12 adds the 44th: raw
player hotbar-tool/fist punches return before damage against registered
Grudgelands mobs, while server-owned ability punches continue; Lane P's
lifecycle hooks add the 45th and 46th, and the round-5 combat-AI pass
(2026-09-17) adds 13 sites. Later additions brought the inventory to 66; the
vegetation-support correction below adds the 67th. The current total is
**67**.
...
quarter-second cliff probe: only ambient `stand`/`walk` states use a 1.5-node
...
`fear_height`; no path search is added. A later playtest correction adds one
marker for the shared bounded support probe: Luanti's `line_of_sight` stops at
every non-air node, so harmless non-walkable vegetation is skipped until the
probe finds real walkable support. Dangerous, unknown and unsupported columns
remain unsafe. The cliff guard and the close-obstacle sidestep use the same
predicate.
...
- **The punch moved out of the branch.** It now sits after both branches and
  ...
  cadence whether or not it continues. At that round's boundary a blocked
  line of sight still cost the swing; the next section supersedes that
  behavior.

In that round `reach` itself was untouched: raising it could not repair the defect (a stopped
...
earlier. The 2026-09-17 ruling in the next section later raises ordinary reach
for close-obstacle parity, not as a cadence fix. Measured on a headless server
(`tools/wp11/run_cadence_probe.sh`):
punches per 10 s against a target receding at walk speed 4.0 go from **1 to
10**, and against a standing target stay at **10**.
At that integration point `mobs/api.lua` had **43 markers**; the current
inventory is recorded in the vendored-mod row below.

## Round 5 — raw player-punch veto (2026-09-17)

One new marker in `mob_class:on_punch`, after the native swing-input seam and
before protection or damage: a player hotbar weapon, ordinary tool or fist
cannot damage a mob registered by `grug_mobs`. Player damage against those mobs
comes only from the authoritative swing/skill transaction; its claimed swing
and cast punches carry the existing authoritative markers and pass through.
Vanilla mobs_redo entities, non-player punches, NPC/trader cancellation and the
separate player-vs-player path are unchanged. This supersedes the raw ordinary
tool/fist mob-damage portion of the older WP38 proportional-melee patch while
leaving its PvP path intact. At that integration point `mobs/api.lua` had
**44 markers**; the current inventory is recorded below.

## Round 5 — progression lifecycle hooks (2026-09-17)

Two new markers keep progression bookkeeping on mobs_redo's existing lifecycle
...
entities are unchanged. At that integration point `mobs/api.lua` had
**46 markers**; the current inventory is recorded below.

## Round-5 combat AI — obstacles, knockback and collision (2026-09-17)

Thirteen new markers implement one close-obstacle state machine plus two
combat-physics rulings:

- `do_attack()` and `stop_attack()` clear the blocked-LOS timer and active
  sidestep at chase boundaries. `smart_mobs()` accepts a close-obstacle forced
  pass after the separate one-second LOS timer; if `find_path` returns no
  usable route it arms a 0.5-second perpendicular sidestep and alternates the
...
`mobs/grug_obstacle.lua`.** The current inventory is 67 markers, as recorded
...
| `mods/BASE/player_api` | minetest_game `mods/player_api` | `b5243f3` | LGPL-2.1+ | Obsolete offline-player API wrappers removed; see fresh-server cleanup above. `api.lua` exposes one `register_control_animation_override` hook inside its existing globalstep, allowing server-owned held actions to select `stand`/`walk` instead of the generic LMB `mine` pose without a second connected-player scan. 2 `GRUG PATCH` markers. |
