...
| [`inventory_equipment.md`](inventory_equipment.md) | Character screen, weapon/offhand and two trinket slots, hand count, separate cultural/PvP-special metadata channels, bags and 3×3 recipe/workbench rules. | **decided**; slot mechanics and the Round 11 item families are delivered, while REPAIR remains pending. |
...
| [`skill_trees.md`](skill_trees.md) | The WP11 talent design: two trees of two chains and twenty-eight ranks per class, one point every two levels, tier and hard-chain gating, keystones that add or replace a button, one reachable capstone, the sixty-four talents, the data model, the respec seam and the four implementation lanes. | **decided, revision 2** (2026-09-16); X1, X2 and X4 plus Round 11's targeted Ironbound/Unbroken armor slice are implemented, while the remainder of X3 is open. |
| [`scout.md`](scout.md) | The fourth class (Scout): mana, leather, bow/dagger/sword/greataxe routes, a four-ability base kit, ballistic arrows, Sprint's bounded speed exception and stealth deferred whole to §8. | **decided** (2026-09-16); GEAR/ART dependencies are delivered and the SCOUT consumer package is open. |
