...
| **Trees** | **Quarry** (bow) and **Veil** (blade and shadow) — `skill_trees.md` §2.7/§2.8, names `skill_trees.md` §6.7 |
...
- **Chance per tick** inside the band:
  ...
  - the ruling's case, a level-40 Scout against a level-60 mob or guard:
    ...
    one number `skill_trees.md` §6.15 recommends keeping even if the others move.
...
| **Character visuals** | the leather overlay line, and a `hidden` composition input for stealth (§6.2, §6.3) | `character_visuals.md:71-73`; `grug_visuals/apply.lua:269-300` |
...
- What the estimate got right: no new profession, no new resource system if
  `skill_trees.md` §6.4(a) is taken, no new damage mechanic (ruling 7's "no poison" is what
