...
crit chance, slows, escape moves. `classes.md:470-482` planned poison as "the
...
|---|---|---|---|---|
| **Loose** | cast | 1 arrow | none (ammo-limited) | Requires a bow in the weapon slot. A ballistic arrow along the cast-time crosshair, 25 m, initial impulse from bounded draw time, gravity supplies the trajectory (`items_crafting.md` §9, `classes.md:282-285`). Damage = the bow's full-charge value + ranged bonus. A miss still spends the arrow. |
| **Gut** | swing | 25 focus | swing, no charge | On a completed landed swing with enough focus, the total is `floor(weapon damage × 1.4) + melee bonus` instead of the plain hit — the Scout's focus dump, deliberately below the Warrior's ×1.5 (`classes.md:420`) because the Scout wears leather. |
...
Fireball is straight and gravity-free (`classes.md:275-281`). Everything else
...
(`grug_projectiles`, `classes.md:282-283`).
...
`classes.md:434-436`: "Fireball pays with mana instead of a cooldown…
...
(`combat_stats.md:310-315`), with the 25 m soft de-aggro as the release valve
(`:328-330`). The mob-pressure task card of the same session proposes making
...
| **PvP (WP41)** | whether stealth hides a Scout from players, and whether stealthing tags | `combat_stats.md:270-289`; §6.6 |
