...
| Read-only reviewer cannot run a KAT whose SHA fallback writes `/tmp` | the review sandbox cannot write `/tmp` | the runner accepts an in-memory digest path; otherwise the reviewer verifies the implementer's immutable digest evidence instead of writing a fallback file |
