...
> **Record only (2026-09-15).** This is the research note of an
> output-preserving mapgen throughput candidate that was deliberately never
> merged (branch tip 698b41db, kept as tag `archive/wp40-mapgen-throughput`
> together with its 385 profiling evidence files). Its mixed timings led to
> the resource-root sampler, which main adopted in 4403f018. The code changes
> described below are not in main.
