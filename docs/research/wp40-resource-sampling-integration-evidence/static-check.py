...
changed=sorted(set(subprocess.check_output(['git','diff','--name-only','a87b58c'],cwd=repo,text=True).splitlines()))
changedlua=sorted(set([f for f in changed if f.endswith('.lua') and (repo/f).exists()] + [str(f.relative_to(repo)) for f in (repo/'tools/wp40/resource_sampling').glob('*.lua')]))
subprocess.run(['/home/jan/projects/grudgelands/tools/bin/luac51','-p']+sorted(set(production+changedlua)),cwd=repo,check=True)
print('Parser PASS',len(production),'production files;',len(changedlua),'changed Lua files')
for f in changedlua:
 out=subprocess.check_output(['/home/jan/projects/grudgelands/tools/bin/luac51','-l','-p',f],cwd=repo,text=True)
...
subprocess.run(['/home/jan/projects/grudgelands/tools/bin/luac51','-p','/tmp/grug-sampler-stage-check/mods/MAPGEN/grug_mapgen/wp40/r6_settlement.lua'],check=True)
