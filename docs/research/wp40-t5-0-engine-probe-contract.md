...
| 13 | `grug_mobs` camp/banner LBMs | `grug_mobs` | `mods/ENTITIES/grug_mobs/camps.lua:733`, `:755` | metadata and node timers only, never content | **no** | as row 8; and `grug_mobs` never sets `on_map_load`, so mobs use the ABM branch (`mods/ENTITIES/mobs/api.lua:4239-4252`), never the LBM branch (`:4226-4238`) |
