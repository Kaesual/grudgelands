...
| `x_farming/` | https://bitbucket.org/minetest_gamers/x_farming.git | Crop breadth (barley, rice, soybean, coffee, strawberry, cocoa, obsidian wart), six-slot rustic stove, sun-dependent drying rack, sickle harvest/replant and biome-bound crop placement — the Round 8 cooking-presentation and plant-breadth reference (`docs/research/plants-and-potions-reference.md` §4). Pinned at `ac5f69d5103d4af841b1a5ed3a0b49a4e19d0c84` (approved by the user 2026-09-18) | LGPL-2.1-or-later code; runtime textures/models/sounds per-file CC BY-SA 4.0/3.0, CC BY, CC0 in `LICENSE.txt` — 59 sounds source-resolved, 81 sounds, 12 MTS schematics, 19 `.blend`, 2 `.xcf` and 5 screenshots NOT per-file cleared (import allow-list only) |
| `farming/` | https://codeberg.org/tenplus1/farming.git | Crop and dish breadth (grains, vegetables, berries, many dishes), grouped-ingredient recipe syntax and the elapsed-time wet/dry soil crop timer — read-only breadth reference for Round 8 cooking and plants (`docs/research/plants-and-potions-reference.md` §2.1/§3). Pinned at `1a918c7ae3778c3406ad91de6b83a174facad8f0` (approved by the user 2026-09-18 with the deny-list) | MIT code; textures per-file/glob CC0, CC BY 3.0, CC BY-SA 3.0/4.0, MIT (`license.txt`). **DENY-LIST, never import:** `farming_chapathi.png`, `farming_gulab_jamun.png`, `farming_kitkat.png`, `farming_laddu.png`, `farming_samosa.png` (CC BY-NC-SA 3.0), `screenshot.jpg` (no row), and the AFL-1.1 block's `farming_asparagus*`, `farming_eggplant*`, `farming_spinach*`, `farming_ginger*` files (code and textures, licence scope unresolved) |
