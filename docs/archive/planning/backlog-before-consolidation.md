...
Archived 2026-09-23 from `BACKLOG.md` at `d6937b31`.
...
tracked in [BACKLOG](../../../BACKLOG.md). Original file SHA256: `60893d3e30a3c2bd57be69afb444bf854249e741f8e832d40c9b9dfc9fd045ea`.
...
portable parity and native registration gates PASS. Merged to main, locally
...
## Round 10 delivery checkpoint

EQUIP, GAME, WORLD, ART, CAP, FARM and MAP-B are delivered on main and
independently clean. Final integrated interpreter and engine gates pass;
synchronization and push are complete. User GUI acceptance remains pending.
These subpackages do not change the whole-WP readiness count below.

## Round 11 execution checkpoint

**Delivered for the next playtest (2026-09-20).** SPEC, GAME, WORLD-CAP,
AFF-GEAR, ART, COMBAT, FARM, REPAIR, SCOUT and its trader slice are independently
clean and integrated. The [completion record](../../../docs/research/round11-completion.md)
records package heads, review corrections and bounded final gates; the
[playtest checklist](../../../docs/research/round11-next-playtest.md) owns GUI acceptance.
The user waived all PUC runtime this round; plain-5.1 syntax/static gates and
minimal targeted LuaJIT checks remain. WP-Scout is complete; larger
WP11/WP14/WP22/WP24/WP44/WP46 scopes remain open. WP22's delivered money-only
service follows [durability_repair.md](../../../docs/design/durability_repair.md);
material/profession-specific repair redesign stays in the backlog.
...
| WP17 | Travel: waypoint nodes at starts, capitals and authored zone hubs; visit-unlock and travel formspec; `/unstuck`; and the claim-bound Home Stone from `docs/design/housing.md` §4. Home stores a stable claim id, has no capital fallback and is rebound only by interacting with an active owned Claim Stone. It also **implements `docs/design/boats.md`**: the always-craftable five-wood base boat, the improved boat plus the permanent Improved Boat unlock the shipwright teaches from level 30, item↔entity lifecycle, exactly one player per boat with no mob or NPC passenger, free pickup/placement with the driver-only exception, the 24-hour decay of an unused boat, 4/8 nodes per second as entity velocity, and eject-on-damage through both seams (the boat entity's `on_punch` and the central `grug_core` HP-change hook). It also ships the three ocean-danger prerequisites that contract depends on: the mobs_redo attack-cadence patch of `combat_stats.md` §4, the Kraken Guard retune of `biomes_mobs.md` §3 (`run_velocity` 8.8, `view_range` 40) and the **position-dependent pursuit state** of `world.md` §2b — relentless while the guard itself stands in a deep-ocean column, the complete §4 leash/evade model in shelf water, planned water and dragon channels, switched at the guard's own position and never reversed once it evades. That third piece **replaces shipped behaviour**, and it is three fields, not one: `_grug_no_leash = true`, the mob's own 200-node coastal `LEASH_SLACK`/`strayed()` special case and `_grug_soft_deaggro = false` (`mods/ENTITIES/grug_mobs/kraken.lua:10`, `:37`, `:38`) all retire as blanket exceptions and come back position-dependent — suspended inside a deep-ocean column, ordinary outside it. Missing the soft-de-aggro half breaks the contract in both directions: left global, the guard keeps 8.8 on the shelf instead of dropping to walking speed at 25 m; removed globally, an 8 nodes/s boat outruns a walking guard on the open sea. Deep-ocean-only spawning needs no new mechanism — `_grug_spawn_check` is already `grug_core.open_sea_at` (`kraken.lua:31`), which WP40 narrows to `deep_ocean`. **The cadence patch changes every mob's feel, so this WP's runtime test covers the whole roster, not only the water.** The shipwright NPC itself is WP13 village content; until it stands, the improved boat is unobtainable while the base boat is not | open; design-complete — the playable-boat contract is decided (2026-08-13, `docs/design/boats.md`) | WP24, WP40 |
...
| WP19 | Combat feel & kit tuning: global cooldown (1 s), soft target lock (~8 s), Mighty Blow as rage dump, Hamstring, Fireball mana-limited, Frost Nova pivot (12 s + slow), Power Word: Shield (absorb via hp modifier), visible race passives | ✅ Original WP19 delivery: GCD + separate enemy/ally memories, tuned kits and race passives. Later combat packages retired the GCD and hostile-memory aim; Round 6 makes Fireball cost 6% base mana with a 1 s server cast cadence/no cooldown and baseline-weapon-plus-spell-power fitted damage, and makes every friendly skill resolve pointed ally → valid ally memory → self. Mighty Blow remains the rage dump, Hamstring the slow, Frost Nova the root→slow pivot and PW:S the central absorb. Runtime tested 2026-08-07; current rules: `docs/design/classes.md` | WP4 |
| WP20 | Persistent same-faction parties of 2–10; indefinite offline membership/leadership, inviter-bound invitations, management tab and optional name/HP HUD. No party-specific XP, quest-credit, loot or tap rules | ✅ Round 14: persistent groups, management and HP HUD; `docs/design/parties.md` | WP4 |
...
| WP22 | Durability and repair: effect loss at zero durability, NPC repair for ledger money and tier-scaled costs; preserve the 3000/6000 combat-event budgets where applicable. Runtime-calibrate the WP29-authored dig-time/durability table (the B22 frame, `items_crafting.md` §3.0.4) and the usable-block targets for all six picks, including the fixed ×4/×6/×8/×10 under-tier destruction penalties. Do not derive speed or wear from the retired engine `leveldiff` coupling | in progress: Round 11 delivers slow wear, retained broken items, explicit reference prices and money-only repair at every city profession trainer; the future Housing station provider seam is available. Six-pick runtime calibration and deferred material/profession repair redesign remain. Specs: `durability_repair.md`, `items_crafting.md` and `economy.md` | WP5, WP7, WP43, WP44 |
| WP23 | Apex world bosses: populate the two offshore level-60 dragon islands, The Wyrmglass Crown and Stormscale Summit, with equivalent authored arenas, telegraphs, hoards, regional variants, reset/participation cleanup, separate 24-hour loot lockouts and persistent 30-minute respawn plus 60-second warning. Both islands stay contested; each requires WP17's playable boats and their authored route, a WP40 island/map foundation, WP13 structures, WP34 renewable apex resources and WP41 PvP. The base boat alone reaches either island, so no part of this WP waits for the shipwright or the improved boat | open; design-ready encounter rules, no design blocker left — the boat contract was decided 2026-08-13 (`docs/design/boats.md`); waits only on the listed implementation foundations Reviewed Round-9/10 dragon runtime and behavior corrections are present; complete encounter infrastructure and boat/PvP/apex dependencies remain open. | WP13, WP17, WP34, WP40, WP41, WP43 |
...
| WP26 | Two-slot furnace and universal alloy chain: port the compatible dual-input furnace and implement Bronze, Iron, Steel (with mined Coal), Silversteel, Embersteel and Abyssal Steel exactly as `items_crafting.md` specifies. It owns processed universal bars only; G1/G2, culture finishes, PvP specials, trinkets, claims, kings and dragons remain in their dedicated WPs | ✅ new mod **`mods/ITEMS/grug_smelting`** (2026-09-16): the LotT dual furnace ported as CODE only (node pair `grug_smelting:dual_furnace`/`_active`, two material slots plus a separate fuel slot, node-timer driven, either-order matcher, one of each input consumed per output), the five §3.2 cooking recipes, the five §3.3 alloys, the twelve §3.4 storage pack/unpack pairs derived from `PROCESSED_MATERIALS`, and the station's own T1 craft recipe (2 Copper Bars + 1 Tin Bar around a `default:furnace`). `grug_smelting.RECIPES` is the public read surface that closes the §3.8 blind spot: `grug_traders`' craft walk sees only `normal`/`cooking` engine recipes, so `grug_traders/audit_alloys.lua` walks the alloy chain with the same price resolution and error-level report, with an overpriced-recipe negative test proving it fires. No price added or changed, no gear/pick/gem recipe, no new item or node beyond the furnace pair. Cook times (3 s smelt; 4/6/8/10/12 s alloys) are WP26 calibration and explicitly not frozen; a real server produced the first Bronze Bar after 4 s and the first Steel Bar after 6 s. `lottblocks` media is CC BY-SA 3.0 and is NOT shipped -- the two front faces are re-skinned from the vendored minetest_game fronts. Gates: `tools/wp26/smelting_kat.lua` (identical under LuaJIT and PUC 5.1, five-mutation gate), a startup self-audit of all 23 registrations, an in-engine smelt probe, and a boot on the user's world seed whose warning set is identical to the same boot without the mod. **NOT user-runtime-tested yet** (task card §8); note: `docs/research/wp26-implementation.md` | WP43 |
| WP27 | Armor base recipes: register the twelve metal/cloth/leather shapes on the one shared equipment ladder; apply the decided T4–T6 G2 material rotation and cultural-visual metadata seams without creating parallel cultural catalogs. Every base recipe remains universally craftable; professions add enchantments or special finishing | open; specs: `items_crafting.md` armor tables and `inventory_equipment.md` Round-10 EQUIP supplies universal base armor and ART its visuals; retain whole-WP acceptance and remaining dependencies separately. | WP26, WP43 |
...
| WP31 | Mounts: four riding tiers, persistent ownership and ephemeral controller/visual, damage dismount, geographic flight rules, capital-only trainers, first-person owner hiding, untimed status, nominal one-node land steps and mounted-attack refusal. WP44 later recalibrates the accepted interim prices. | in progress: reviewed Round 10 foundations plus Round 11 shared rider/mount orientation, open stables and animated grounded displays are delivered. GUI acceptance and the WP44 economy calibration remain; this is not whole-WP completion. | WP28, WP40, WP44 |
| WP32 | Farming: all 17 crop families have seed, wet-soil growth, pause/resume and harvest/replant mechanics; MAP-B supplies world acquisition, secondary/field soils and sea/cave placement. Later active-claim integration retains the exhaustive ten-node growth audit. | in progress: all seventeen crop lifecycles, legal-ground farming and world acquisition are delivered. Round 11 adds distinct seed icons, seven hoes, water buckets, halved wild-source density and bounded depletion renewal. GUI acceptance remains; future Claim Stone integration belongs to WP24. | WP10, WP24, WP33 |
...
6. **T5 — reusable projectile foundation and Fireball.** The public
   ...
   cleanup and bounded active counts. Fireball spends 6% of the base mana pool
   after a successful spawn, has a 1 s server cast cadence with no cooldown,
   travels straight at 20 m/s for at most 20 m, ignores allies and drops, has
   no homing/gravity/splash, and deals baseline weapon damage + spell power
   through the shared damage fit once.
...
and spends 6% base mana on a miss. Its 1 s server cast cadence is not a
cooldown. At most eight Fireballs per owner/session are active;
...
**Round 6 integration (2026-09-17):** the shared player-pool/damage model,
effective tooltips, maximum-pool Character/Help pages, total friendly-skill
fallback and 5% suffocation are shipped. `grug_core.status`, its top-right text
list and `grug_food` delivered the first recovery-buff minimum; Round 7's Food
v2 core now supersedes its three-quality model. The
...
modifiers and shared consumable level gates are shipped. Mana regeneration is
level-linear, and a per-world marker skips completed start-area preload on
later server starts. Round 8 Cooking v1 now supplies the real six-tier dish
tables, raw/furnace routes, raw-food tiers and plant inventory items; world
placement for those plants remains separate.

**Round 8 integration (2026-09-19):** the shared profession UI, progression,
trainer and station authority now carries Cooking v1 and Alchemy v1; five
primary catalogs remain for Round 9. Per-viewer player nametags, the first new
day/night mob families, both dragons, six kings with their royal guards and
continuous capital boundary rings are shipped. Four coast profiles, sand,
shallow strata and connected cave mouths are live; MAP-B plant placement,
soils and crop soil moved to Round 9.

**Round 10 delivery (2026-09-20):** the independently reviewed
EQUIP, GAME, WORLD, ART, CAP, FARM and MAP-B subpackages supersede the earlier
catalog/plant-placement deferrals above. Six capitals now provide eight profession
trainers plus a separate Riding Trainer, seven public stations, four mount
displays and three gear displays each (48 profession trainers, six Riding
Trainers, 42 stations, 24 mount displays and 18 gear displays across the capitals).
Wild Cooking sources, ash/moss soils, actual field soil and shallow-sea reefs are
implemented, with a complete seventeen-family farming loop on legal ground.
Final interpreter parity, main delivery, synchronization and push are complete; user acceptance
uses [the Round-10 checklist](../../../docs/research/round10-next-playtest.md). WP13 retains
the remaining 100-anchor POI roster and the decided 24-node versus implemented
16-node `bandit_frontier` building-core-width mismatch; the design stays at 24.

**Active and ready next:**

1. **WP13 — world structures**, selected by the user after WP40 completion.
   All six starts and capitals stand; their trainers, kings, royal guards and
   final boundary rings are populated. The Round-10 consolidated fresh-world
   checklist is the next GUI gate. The
   remaining scope is the 100-anchor POI roster, including villages,
   shipwright plots, camps, outposts, dragon arenas and apex mining camps.
2. **WP5 — loot/affixes** and **WP44 — economy rebase** are independently
   ready; both use the shipped material foundation, and WP5 has an executable
   task card. **WP26 — universal bars/furnace is shipped** (2026-09-16), so
   WP27/WP29 and WP10 no longer wait on it.
3. **WP37 — surface density**, **WP14 — offhand/carried light**, **WP21 — recovery/rest** have
   shipped prerequisites. WP8 quests and WP20 parties are delivered in Round 14. WP11 X1–X4 are implemented; measured respec-price calibration still depends on WP44.
...
browser-local Lua-5.1 user results, exact evidence limits and independent review.
It supersedes the old “R8 still open” current-status prose; historical milestone
records keep their original source/algorithm identity.

**Blocked or ordered work:**

- **WP5** is design-complete (A2/A6 decided 2026-08-13,
  `items_crafting.md` §§6b.4/6b.7) and ready behind shipped
  WP1/WP3/WP43. **WP10** is design-side ready (A1/A3/A4/A5/E21 decided
  2026-08-13, R22 decided 2026-09-17) and follows WP44; WP26, WP33 and WP43
  are shipped, as are Round 6's status-list and food-tier foundations.
- **WP27 → WP29 → WP28** is the replacement-before-deletion equipment/tool
  chain, and both of its prerequisites (WP26, WP43) are now shipped, so WP27 is
  startable. WP30 then consumes WP29 and WP44.
- **WP34** still waits for WP13 structures and WP44 economy; WP40/WP43 are
  shipped. WP33 gathering is already delivered. WP34 is not independently ready.
- **WP24** consumes WP40's exact housing masks/capacity, WP43's bar ids and
  ...
  claim service and implements the decided boat contract
  (`docs/design/boats.md`) together with its three mob-side prerequisites —
  attack cadence, Kraken retune and the position-dependent pursuit state that
  retires the mob's shipped no-leash/200-node special case.
  **WP23** therefore carries no design blocker any more and waits only on
  structures, map, boats, contested PvP and resource supply.
- **WP41** still waits for WP5 and implements the exact §15 transaction;
  WP39/WP40 are shipped. **WP42** follows WP13/WP41. **WP9** still waits for
  WP41; WP8 is shipped. WP12 atlas is shipped without travel; WP17 owns later
  waypoint integration. WP40 no longer blocks these packages.
- **WP31** has integrated, independently clean gameplay, capital-service and art
  subpackages; GUI acceptance remains. WP44 still recalibrates its four prices.
- **WP32** has integrated, independently clean crop mechanics and art; MAP-B and
  active-claim integration remain, with the latter still dependent on WP24.
...
the old names and semantics remain only in shipped-history prose. Development-
era migrations were removed under the standing fresh-server instruction.
...
review gates but still needs a material-focused fresh-world GUI/runtime pass.
No old-world migration test applies in fresh-server development mode. WP45's existing- and new-character runtime passes
were green on 2026-09-03; an incomplete character's mid-flow reconnect remains
headless-tested only. WP25, WP35 and WP36 retain their historical
...
`territory_at/zone_at/difficulty_at/mob_level_at` (the historical WP2 API;
`difficulty_at` was later removed, ring anchors per
...
griefing) moved to `TODO-design-spawn-safety.md` — resolved 2026-08-13 for
the permanent WP40/WP13 start settlements and folded into `world.md` §2 R1
(full-envelope-plus-apron protection, fail-closed indirect mutation, no
rollback); the TODO file is deleted and the legacy WP18 platforms simply
retain their shipped behavior until WP40/WP13 replace them.
...
`do_custom`**, never `core.after`; the **chase model** (Rounds 18–19 ordinary mobs: 15 s without incoming damage plus
current-target horizontal movement; no distance give-up/slowdown; authored encounters retain bounds;
...
Vendored `mobs_redo` carries **62 `GRUG PATCH` sites in `api.lua`**
...
- **Crocodile: one speed 4.6**, no 5.0-in-water bonus (mobs_redo rewrites
...
- **WP20:** no party tap, XP or quest-credit rules. Current per-mob damage/
  effective-heal participants share XP and quest credit independently of groups;
  the former lethal-hit/first-party proposal is superseded (Round 14).
...
- **WP5**: the ×3 **Uncommon is built but never offered** until
  ...
  `grug_traders`; the generated catalog's `_grug_ilvl` is already enforced
  directly as the Weapon-slot minimum level; the
  **leather armor line** (curve is in the generator, `register = false`;
  the shipped rank filter refuses only `rank > max`, and the C10
  decision — 2026-08-13, `items_crafting.md` §3.8 — ships the line as
  the Warrior's light set with WP29's catalog merge); and
  **quality/wear are ignored by buy-back
...
- **WP11**: the **respec price** is decided (user ruling 2026-09-16): **five
  minutes of measured reliable net solo income** at the character's bracket,
  rounded by `economy.md` §4.1's rule, and **the first respec of a character
  is free**. `grug_money.take` is still the API it calls
  (`mods/PLAYER/grug_money/init.lua:122`), and WP44's income measurement is
  what turns the rule into six numbers. X4 currently marks its six executable
  values as a **coordinator placeholder**; they are not measured outputs and
  must be replaced when WP44 publishes the ledger. **This retires the old "5c × level,
  min 25c"** that stood here, and `items_crafting.md` §8.3's "repeatable at
  the class trainer and rising with level" had the trainer struck from it on
  2026-09-16, together with `economy.md` §4 and `world.md` §1
  (`docs/design/skill_trees.md` §7, task 3).
...
`deal_ability_damage` (crit ×1.5, applies via `object:punch` so armor and
XP-on-death keep working; knockback requires an explicit override),
`heal_player`, central dodge roll as
