...
mapfile -t files < <({ git diff --name-only --diff-filter=ACMR 5d850163 -- '*.lua'; git ls-files --others --exclude-standard -- '*.lua'; } | sort -u)
for file in "${files[@]}"; do
  tools/bin/luac51 -p "$file"
  printf 'PARSER PASS %s\n' "$file"
  tools/bin/luac51 -l -p "$file" | rg SETGLOBAL || [[ $? == 1 ]]
done
