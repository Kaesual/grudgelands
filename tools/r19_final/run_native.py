...
root = pathlib.Path(tempfile.mkdtemp(prefix='grug-r19-integration-', dir='/tmp'))
...
probe = game/'mods/r19_integration_probe'
probe.mkdir()
(probe/'mod.conf').write_text('name = r19_integration_probe\ndepends = grug_mobs, grug_map, grug_parties, grug_abilities, grug_inventory, grug_skills\n')
shutil.copyfile(repo/'tools/r19_final/probe.lua', probe/'init.lua')
...
print('\n'.join(line for line in text.splitlines() if 'r19_' in line or 'ERROR' in line), flush=True)
...
assert '[r19_integration] PASS dragon_profiles=2' in text, str(log)
print('PASS', root)
