...
assert(grug_repair.cost(lists.main[2])==nil and grug_repair.cost(lists.main[3])==nil)
local refined=damaged("test:sword",65535);refined.meta.grug_refined="1"
assert(grug_repair.cost(refined)==20)
refined.meta.grug_quality="2";assert(grug_repair.cost(refined)==60)
refined.meta.grug_quality="3";assert(grug_repair.cost(refined)==120)
local q=assert(grug_repair.quote(player,provider));assert(q.total==132 and #q.items==4)
local observed=false
grug_money.register_on_change(function()
	observed=true
	assert(lists.grug_weapon[1].wear==0 and funds==868,"callback observed half transaction")
end)
...
lists.grug_weapon[1].meta.affixes="changed since quote"
local before=copy(lists)
assert(not grug_repair.apply(player,q) and writes==0 and funds==1000 and equal(before,lists))
reset();q=assert(grug_repair.quote(player,provider));funds=1
assert(not grug_repair.apply(player,q) and writes==0 and funds==1)
reset();q=assert(grug_repair.quote(player,provider));fail_at=3;before=copy(lists)
assert(not grug_repair.apply(player,q) and funds==1000 and equal(before,lists))
reset();q=assert(grug_repair.quote(player,provider));position={x=9,y=0,z=0}
assert(not grug_repair.apply(player,q) and writes==0 and funds==1000)
position={x=0,y=0,z=0};faction="throng";assert(not grug_repair.quote(player,provider))
faction="accord";live=false;assert(not grug_repair.quote(player,provider));live=true
npc._grug_socket="other";assert(not grug_repair.quote(player,provider));npc._grug_socket="smith"
npc._grug_socket_role="riding_trainer";assert(not grug_repair.can_open_trainer(player,npc))
npc._grug_socket_role="trainer"
lists.main[1]=damaged("test:unpriced",123)
assert(not grug_repair.quote(player,provider),"unpriced equipment repaired free")
reset();assert(grug_repair.open_trainer(player,npc))
