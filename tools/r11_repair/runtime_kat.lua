...
grug_repair = {
	eligible = function(item)
		...
		return (groups.grug_equip_weapon or 0) > 0 or
			(groups.grug_spellbook or 0) > 0 or (groups.hoe or 0) > 0
	end,
}
grug_inventory = {
	BAG_COUNT = 1,
...
local function fresh_weapon(refined)
	local item = stack("test:weapon")
	item:get_meta():set_int("grug_refined", refined and 1 or 0)
	return item
end

load_runtime()
lists.grug_weapon[1] = fresh_weapon(false)
for index = 1, 2999 do callbacks.outgoing(player, "common:" .. index) end
check(lists.grug_weapon[1].wear < 65535, "common broke before event 3000")
callbacks.outgoing(player, "common:3000")
check(lists.grug_weapon[1].wear == 65535, "common did not break at event 3000")
check(lists.grug_weapon[1].caps.damage_groups.fleshy == 0,
	"broken weapon retained capabilities")

lists.grug_weapon[1] = fresh_weapon(true)
for index = 1, 5999 do callbacks.outgoing(player, "refined:" .. index) end
check(lists.grug_weapon[1].wear < 65535, "refined broke before event 6000")
callbacks.outgoing(player, "refined:6000")
check(lists.grug_weapon[1].wear == 65535, "refined did not break at event 6000")
...
print("r11_repair_runtime_kat\tPASS")
