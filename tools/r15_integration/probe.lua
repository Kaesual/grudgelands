...
local function log(s) core.log("action", "[r15_integration] " .. s) end
...
core.register_on_mods_loaded(function()
 ...
 assert(count(grug_quests.registered_quests)==102 and count(grug_quests.registered_npcs)==30)
 log("catalog PASS quests=102 npcs=30 manifest="..grug_mapgen.wp40.manifest_sha256)
 local records={}
    ...
    if left==0 then
     core.after(0,function()
      ...
      if completed==3 then core.after(6,function()
       ...
       core.request_shutdown("bounded Round 15 integration complete",false,0)
      end) end
     end)
    end
   end)
...
core.after(110,function() error("Round 15 integration deadline") end)
