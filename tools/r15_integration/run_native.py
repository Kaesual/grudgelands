...
root = pathlib.Path(tempfile.mkdtemp(prefix='grug-r15-integration-', dir='/tmp'))
...
for filename, needle, marker in (
    ...
    text = path.read_text()
    assert text.count(needle) == 1
    path.write_text(text.replace(needle, needle+'\n core.log("action", "[r15_integration] consumer '+marker+'")'))
probe = game/'mods/r15_integration_probe'
probe.mkdir()
(probe/'mod.conf').write_text('name = r15_integration_probe\ndepends = grug_quests, grug_mobs, grug_mapgen\n')
shutil.copyfile(repo/'tools/r15_integration/probe.lua', probe/'init.lua')
...
print('\n'.join(line for line in text.splitlines() if 'r15_integration' in line or 'ERROR' in line), flush=True)
assert result.returncode == 0 and 'ERROR[' not in text, str(log)
assert '[r15_integration] PASS three_chunks=3 catalog=102/30' in text, str(log)
