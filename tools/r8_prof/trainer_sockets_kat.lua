...
return function(repo)
	...
	local function projected(key, slot)
		local prepared = {schema = "grug_wp13_settlement_prepared_v1",
			profile = {key = key, label = key, slot = slot}, blueprints = {}}
	...
	for index = 1, #starts do
		local rows = projected(starts[index], "start")
	...
	for index = 1, #capitals do
		local rows = projected(capitals[index], "capital")
