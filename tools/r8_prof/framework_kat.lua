...
return function(repo)
	...
	check(grug_jobs.crafts_in_tier(novice, "cooking") == 0,
		"completed capped threshold was retained as a second grind")
	novice.level = 11
	check(grug_jobs.profession_level(novice, "cooking") == 2,
		"earned tier did not open with the next character band")
	...
	grug_jobs.register_recipe({profession = "cooking", tier = 2,
		...
		hint = "Crafting grid"})
	local predicted = hooks.predict[1](ItemStack("test:dish"), locked)
	check(predicted and predicted:is_empty(), "grid recipe predicted without book")
	grug_jobs.learn(locked, "cooking")
	check(hooks.predict[1](ItemStack("test:dish"), locked) == nil,
		"learned grid recipe was refused")
	local above = hooks.predict[1](ItemStack("test:t2dish"), locked)
	...
	hooks.craft[1](ItemStack("test:dish"), locked, {}, {set_list = function() end})
	...
	line("permission", "locked_refused", "learned_allowed",
		"above_level_refused", "requirement_named", "craft_recorded")
