#!/usr/bin/env python3
"""Draw the Round 11 bow with the real tool and dedicated bow transforms.
...
local out = {}
for _, pose in ipairs({"tool", "bow"}) do
...
def main():
    ...
    BOW = tex("mods", "ITEMS", "grug_gear", "textures",
              "grug_gear_bow_lebethron.png")

    # name -> the rows of its sheet: (caption, sprite, pose)
    SHEETS = [
        ("bow", "grug_gear:bow_bronze -- centre grip, upright arc",
         [("CONTROL  inherited pose=tool (lower-left tool grip)", BOW, "tool"),
          ("ROUND 11 pose=bow (arc midpoint in fist)", BOW, "bow")]),
    ]
