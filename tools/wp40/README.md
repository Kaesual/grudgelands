...
## Current R7 native-input pin refresh

Use this procedure when one of the six R7 NoiseParams, the native allowlist or
the live v7 feature flags changes. Work on final candidate bytes except for the
two deliberately stale identity literals described below. Give every engine
invocation an assigned isolated port.

```sh
git status --porcelain

# 1. After editing the NoiseParams, retain the old NOISE_DIGEST for this call.
#    The expected failure prints the digest generated from canonical_noise_bytes.
luajit -e 'wp40_ffi=require("ffi")' \
  tools/wp40/r7/native_inputs_kat.lua "$PWD"

# 2. Hand-review and replace every noise/flag pin listed below, but retain the
#    old SOURCE_PROJECTION_SHA256. The real load-time constructor prints the
#    newly generated projection digest in its expected failure.
PORT="${PORT:?assigned isolated port required}" nice -n 19 \
  bash tools/luanti_headless.sh 60

# 3. Replace SOURCE_PROJECTION_SHA256 with that generated value, then require
#    the complete native-input and live-constructor paths to pass.
luajit -e 'wp40_ffi=require("ffi")' \
  tools/wp40/r7/native_inputs_kat.lua "$PWD"
PORT="${PORT:?assigned isolated port required}" nice -n 19 \
  bash tools/luanti_headless.sh 60
...
bash tools/wp40/quality/final_micro.sh \
  /tmp/wp40-r7-native-refresh-final
cmp /tmp/wp40-r7-native-refresh-final/puc.tsv \
  /tmp/wp40-r7-native-refresh-final/luajit.tsv
git diff --check
git status --porcelain
```

The canonical NoiseParams bytes and their SHA-256 are generated by
`r7_native.lua`; the source-projection SHA-256 is generated by the real
`r7_manifest.new(inputs)` call during the load-time engine path. The human
copies those two reported values only after reviewing the corresponding diff.
All occurrences of the noise digest, feature token and exact live flag set in
...
does **not** authenticate a call to the returned `.new(inputs)` constructor.
The successful headless load in step 3 is therefore a required part of this
refresh procedure. R5-R8 contracts, TSVs, evidence directories and committed
receipts are immutable history; a refresh creates new R9 evidence instead of
rewriting them.
