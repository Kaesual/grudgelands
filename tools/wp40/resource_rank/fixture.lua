...
return function(repo, expanded)
	...
	check(own.eligible == 6 and own.planned == 2 and own.placed_nodes == 6 and
		own.collisions == 0 and own.shortfall == 0,
		"own frontier-consumed roots differ")
	local short = check(ledger_row("normal_a", -1, -47, -2, 5),
		"short-frontier ledger absent")
	check(short.eligible == 4 and short.placed_nodes == 3 and short.shortfall == 1,
		"short frontier result differs")
	local exhausted = check(ledger_row("normal_b", -2, -47, -2, 5),
		"exhausted-root ledger absent")
	check(exhausted.eligible == 6 and exhausted.planned == 3 and
		exhausted.accepted == 0 and exhausted.collisions == 6 and
		exhausted.shortfall == 6, "exhausted roots lost subsequent shortfall")
