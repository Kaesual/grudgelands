...
grep -q 'exactly two 96-node approach intervals' "$stage1"
grep -q 'expect_failure("warcoast_landmark_contract"' "$test_file"
grep -q 'WP40 T2 WARCOAST-SOURCE-1 oracle passed' "$test_file"
grep -q 'old_channel_columns==11542' "$test_file"
grep -q 'owning-zone clearance envelope drift' "$test_file"
grep -q 'inner clearance 840-64=776' "$test_file"
