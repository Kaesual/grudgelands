...
[[ "$(awk 'END {print NR + 0}' "$derived_changed")" -eq 268 ]] || {
...
[[ "$(micro_field executed_module_population)" == 268 &&
...
awk -F '\t' '$1 == "executed_module" && NF == 2 {print $2}' "$micro_output" \
	| sort >"$executed_rows"
[[ "$(awk -F '\t' '$1 == "source/executed_module_count" && $2 == "268" {
...

{
	...
	printf 'count\tchanged_production_lua\t268\n'
	printf 'count\tdeleted_legacy_lua\t12\n'
