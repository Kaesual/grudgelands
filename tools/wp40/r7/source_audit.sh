...
[[ "$(awk 'END {print NR + 0}' "$derived_changed")" -eq 157 ]] || {
...
[[ "$(micro_field executed_module_population)" == 157 &&
...
awk -F '\t' '$1 == "executed_module" && NF == 2 {print $2}' "$micro_output" \
	| sort >"$executed_rows"
[[ "$(awk -F '\t' '$1 == "source/executed_module_count" && $2 == "157" {
...

{
	...
	printf 'count\tchanged_production_lua\t157\n'
