...
root = pathlib.Path(tempfile.mkdtemp(prefix='grug-r16-integration-', dir='/tmp'))
...
probe = game/'mods/r16_integration_probe'
probe.mkdir()
(probe/'mod.conf').write_text('name = r16_integration_probe\ndepends = grug_quests, grug_mobs, grug_mapgen, grug_map, grug_gear, grug_food\n')
shutil.copyfile(repo/'tools/r16_final/probe.lua', probe/'init.lua')
...
print('\n'.join(line for line in text.splitlines() if 'r16_integration' in line or 'ERROR' in line), flush=True)
assert result.returncode == 0 and 'ERROR[' not in text, str(log)
assert '[r16_integration] PASS armor=' in text, str(log)
print('PASS', root)
