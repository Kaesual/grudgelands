...
return function(repo)
 ...
 function sfinv.get_nav_fs() return "tabheader[0,0;nav;Map;1;true;false]" end
 ...
 local env = setmetatable({sfinv = sfinv, grug_inventory = {}, grug_map = {atlas = atlas},
 ...
 local theme = assert(loadfile(repo .. "/mods/PLAYER/grug_inventory/ui.lua"))
 setfenv(theme, env); theme()
 ...
 has("formspec_version[4]size[10.4,11.1]real_coordinates[false]tabheader[")
 local normal = sfinv.make_formspec(player, context, "", false)
 assert(form:match("size%[[^%]]+%]") == normal:match("size%[[^%]]+%]"))
 local width, height = form:match("image%[0,0;([%d.]+),([%d.]+);grug_map_atlas_world.png%]")
 width, height = assert(tonumber(width)), assert(tonumber(height))
 assert(math.abs(width / height - 9/8) < 0.00001)
 assert(width > 10.08 and height > 8.96, "map uses larger shared window")
 ...
 for _, zoom in ipairs({1, 2, 4}) do
  ...
  for _, x in ipairs({-3600,3600}) do for _, z in ipairs({-3200,3200}) do
   local sx, sy = atlas.world_to_screen(atlas.view(), {x=x,z=z}, 0,0,width*zoom,height*zoom)
   assert(sx == (x+3600)/7200*width*zoom)
   assert(sy == (3200-z)/6400*height*zoom)
  end end
 end
 send({grug_map_zoom_in = true})
 assert(context.grug_map_zoom == 2 and context.grug_map_scroll_x == 500)
 has(("scroll_container[0,0;%s,%s;"):format(width*2,height)); has(";0.42,0.42;")
 local before = sends
 ...
 local image_w, image_h = form:match("image%[0,0;([%d.]+),([%d.]+);grug_map_atlas_world.png%]")
 assert(math.abs(tonumber(image_w)-width*4)<0.00001 and math.abs(tonumber(image_h)-height*4)<0.00001); has(";0.42,0.42;")
