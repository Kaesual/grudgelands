...
root = pathlib.Path(tempfile.mkdtemp(prefix='grug-r17-integration-', dir='/tmp'))
...
probe = game/'mods/r17_integration_probe'
probe.mkdir()
(probe/'mod.conf').write_text('name = r17_integration_probe\ndepends = grug_quests, grug_mobs, grug_mapgen, grug_map, grug_gear, grug_food, grug_home, grug_parties\n')
shutil.copyfile(repo/'tools/r17_final/probe.lua', probe/'init.lua')
...
print('\n'.join(line for line in text.splitlines() if 'r17_integration' in line or 'ERROR' in line), flush=True)
assert result.returncode == 0 and 'ERROR[' not in text, str(log)
assert '[r17_integration] PASS homes=' in text, str(log)
print('PASS', root)
