...
#
# Round 4's copy of wave 3's script, identical but for the main commit and the
# package label it stamps into each file's provenance line. A pass whose
# `<label>_ignored` was non-zero never reaches this script: `run_capital.sh`
# refuses to print such a digest into `overlay-digests.txt` at all.
#
...
main_commit="${3:-dfb32cd5}"
package="20260916-streets-r4"
