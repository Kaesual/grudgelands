...
CHANGED=(
	...
	tools/wp13/visuals_order_kat.lua
)
...
echo "== the art is reproducible from its generator, and covers what it promises =="
# The generator asserts per-face coverage of every overlay and exits non-zero
# with a COVERAGE line when a slot leaves a face it promised uncovered -- or
# closes one it promised to leave open (the helmet's face opening).
before="$(sha256sum mods/PLAYER/grug_visuals/textures/*.png | sha256sum)"
if python3 tools/wp13/gen_character_visuals.py; then
	echo "overlay coverage PASS"
else
	echo "overlay coverage FAIL (see the COVERAGE lines above)"
fi
...
echo "== per-face coverage of every armor overlay =="
python3 - <<'PYTHON'
import sys
sys.path.insert(0, "tools/wp13")
from PIL import Image
import gen_character_visuals as g

overlays = {}
for line in sorted(g.LINES):
    for slot in ("head", "chest", "legs", "feet"):
        overlays[line, slot] = Image.open(
            "mods/PLAYER/grug_visuals/textures/grug_visuals_%s_%s.png"
            % (line, slot)).convert("RGBA")
for (line, slot), image in sorted(overlays.items()):
    px = image.load()
    cells = []
    for group, faces, rule, value in g.REQUIRED_COVER[slot]:
        for face in faces:
            x, y, w, h = group[face]
            op = sum(1 for j in range(y, y + h) for i in range(x, x + w)
                     if px[i, j][3] > 0)
            cells.append("%s %d/%d %s" % (face, op, w * h, rule))
    print("%s_%s: %s" % (line, slot, "; ".join(cells)))
print("complaints:", g.check_coverage(overlays) or "none")
PYTHON
