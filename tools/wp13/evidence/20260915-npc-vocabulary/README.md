...
| `kat.sh`, `kat.txt`, `kat/` | `start_npcs_kat` under both interpreters (byte-identical), the WP40 `vendor_fixture` under both, the `micro_kat_fixture` trader-projection check (with the line it reached, so the gate is not a bare absence test), and the six start identities. `kat/micro-kat-fixture.txt` has LuaJIT's ASLR address replaced with `0xADDRESS`, so the capture is reproducible and `files.sha256` can actually verify it |
...
**The three scripts rewrite the files in this directory** (that is how the
evidence is produced), so a reviewer's rerun dirties the tree and is restored
with `git checkout -- tools/wp13/evidence/20260915-npc-vocabulary/`. Every file
they write is byte-reproducible on the same tree, which is what `files.sha256`
is for: check it with

```bash
grep -v '^#' tools/wp13/evidence/20260915-npc-vocabulary/files.sha256 |
	sha256sum -c
```
...
| race-flavoured lines added | **42** (seven keys × six races) |
