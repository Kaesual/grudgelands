...
| `highcourt/avenue-digest-<seed>.txt`, `<capital>/<label>-digest-<seed>.txt` | the built road, rampart, gate and CORNER overlays as READ BACK OUT OF THE FINISHED MAP; `run_highcourt.sh` and `run_capital.sh` gate against these. `corner` arrived with Nhal Veyr on 2026-09-16 and is the four places two wall runs meet, which no other region contains — `wall.lua`'s corner reconciliation was ungated until it existed | see "The values that are not frozen forever" |
