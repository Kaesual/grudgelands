#!/usr/bin/env bash
# The single bounded final-byte process for the seam generalisation: every WP13
...
out="$repo/tools/wp13/evidence/20260915-seam-generalisation/final-micro"
cd "$repo"
rm -rf "$out"
mkdir -p "$out"
...
( cd "$repo" && sha256sum \
	tools/wp13/evidence/20260915-seam-generalisation/final-micro/micro-luajit.tsv \
	tools/wp13/evidence/20260915-seam-generalisation/final-micro/micro-puc51.tsv ) \
	>"$out/digests.txt"
