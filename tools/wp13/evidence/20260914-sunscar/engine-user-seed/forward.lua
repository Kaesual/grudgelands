...
-- A start's authored cells span 127 nodes on each horizontal axis and 27
-- vertically. Owners are 80 nodes on a side, so 127 nodes touch TWO OR THREE
-- owners per horizontal axis and 27 nodes touch one or two vertically: the
-- structural ceiling is 3 x 3 x 2 = eighteen per start, and it grows with the
-- roster instead of being re-typed.
--
-- The first version of this bound said "at most two owners per horizontal
-- axis, so sixteen for two starts". That is arithmetically wrong -- 127 > 80
-- -- and it held only because the two starts it was written for happened to
-- fall kindly. The third start put 25 owners on the boundary seed and the
-- ceiling refused them; the ceiling was the error, not the corpus.
assert(#owners <= 18 * #starts,
...
assert(#owners <= 18 * #starts + 2, "WP13 control population is unbounded")
...
core.register_on_shutdown(function()
	...
	for _, start in ipairs(starts) do
		...
		-- The main route runs the way the settlement says it runs: the Accord
		-- starts leave north and the Throng starts south, so the corridor
		-- comes from the blueprint's own `main_street` landmark rather than
		-- from an assumed +z.
