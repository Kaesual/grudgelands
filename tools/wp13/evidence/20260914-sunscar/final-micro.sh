#!/usr/bin/env bash
# The single bounded final-byte process for the Sunscar increment: the three
...
out="$repo/tools/wp13/evidence/20260914-sunscar/final-micro"
cd "$repo"
mkdir -p "$out"
...
( cd "$repo" && sha256sum \
	tools/wp13/evidence/20260914-sunscar/final-micro/micro-luajit.tsv \
	tools/wp13/evidence/20260914-sunscar/final-micro/micro-puc51.tsv ) \
	>"$out/digests.txt"
cat "$out/digests.txt"
