#!/usr/bin/env bash
# The single bounded final-byte process for the Highcourt fill package: every
...
out="$repo/tools/wp13/evidence/20260915-highcourt-fill/final-micro"
cd "$repo"
rm -rf "$out"
mkdir -p "$out"
...
( cd "$repo" && sha256sum \
	tools/wp13/evidence/20260915-highcourt-fill/final-micro/micro-luajit.tsv \
	tools/wp13/evidence/20260915-highcourt-fill/final-micro/micro-puc51.tsv ) \
	>"$out/digests.txt"
