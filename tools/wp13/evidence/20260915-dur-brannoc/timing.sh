#!/usr/bin/env bash
# Build time of Dur Brannoc against the capitals contract's section 2.3 budget,
# three runs under each interpreter. A timing is the one thing the two
# interpreters must NOT agree on, so this writes to stdout and is never part of
# the final micro pair.
set -euo pipefail
export LC_ALL=C
