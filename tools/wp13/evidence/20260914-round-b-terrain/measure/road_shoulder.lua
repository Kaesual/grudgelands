...
-- rows on the approach, the smallest |dx| at which a biome decoration may host
...
for index = 1, #src.routes do
	...
	if anchor then
		...
		for _, offset in ipairs({80, 100, 120}) do
			...
			file:write(table.concat({anchor.id, route.id, "approach", offset,
