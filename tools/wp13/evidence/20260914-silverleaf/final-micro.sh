#!/usr/bin/env bash
# The single bounded final-byte process for the Silverleaf increment: the
# three WP13 fixtures in one interpreter process, run once under LuaJIT and
# once under the engine's bundled PUC 5.1 build, with the input set hashed
# before and after so the two runs provably saw the same bytes.
set -euo pipefail
export LC_ALL=C
repo="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd -P)"
out="$repo/tools/wp13/evidence/20260914-silverleaf/final-micro"
cd "$repo"
mkdir -p "$out"
...
( cd "$repo" && sha256sum \
	tools/wp13/evidence/20260914-silverleaf/final-micro/micro-luajit.tsv \
	tools/wp13/evidence/20260914-silverleaf/final-micro/micro-puc51.tsv ) \
	>"$out/digests.txt"
cat "$out/digests.txt"
