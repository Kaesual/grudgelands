#!/usr/bin/env bash
# Static gates for the WP13 Silverleaf increment, exactly the set the previous
# increment's evidence ran: parser and SETGLOBAL per changed file and
...
echo "== parser and SETGLOBAL on every Lua file changed by these fixes =="
...
echo "== five plain-5.1 sweeps, scoped to the Lua these fixes changed =="
sweeps "${CHANGED[@]}"
