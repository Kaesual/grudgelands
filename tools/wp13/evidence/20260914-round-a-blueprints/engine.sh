...
# frozen bytes of the 2026-09-14 round-A playtest fix round. Two seeds, one after the other,
...
cd "$repo"
here="tools/wp13/evidence/20260914-round-a-blueprints"
launcher="$here/luanti-flatpak-launcher.sh"

run_seed() {
	local label="$1" seed="$2" port="$3"
	local out="/tmp/grug-wp13-round-a-$label"
	rm -rf -- "$out"
