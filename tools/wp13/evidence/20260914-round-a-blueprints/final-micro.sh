#!/usr/bin/env bash
# The single bounded final-byte process for the WP13 starts integration merge:
# the three WP13 fixtures in one interpreter process, run once under LuaJIT and
...
out="$repo/tools/wp13/evidence/20260914-round-a-blueprints/final-micro"
cd "$repo"
mkdir -p "$out"

INPUTS=(
	# `library_kat` reconstructs the node registry through
	# `tools/wp13/stub_registry.lua`, which executes the node-registering
	# sources of the vendored mods; `grug_nodes` is the one of them this
	# round edited, so it is hashed with the rest.
...
( cd "$repo" && sha256sum \
	tools/wp13/evidence/20260914-round-a-blueprints/final-micro/micro-luajit.tsv \
	tools/wp13/evidence/20260914-round-a-blueprints/final-micro/micro-puc51.tsv ) \
	>"$out/digests.txt"
cat "$out/digests.txt"
