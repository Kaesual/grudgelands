#!/usr/bin/env bash
# Static gates for the WP13 Highcourt districts increment: the plain-5.1
# parser, the SETGLOBAL count per touched file and tree-wide, and the five grep
# sweeps of docs/research/luanti-lua.md -- scoped first to the files this
# package touched and then to the whole WP13 surface.
set -euo pipefail
export LC_ALL=C
...
echo
echo "== the same five sweeps over the touched files =="
run_sweep "1 goto/labels" '(^|[^[:alnum:]_.:])goto[[:space:](]|::[A-Za-z_]+::' "${touched[@]}"
run_sweep "2 escapes" '\\u\{|\\x[0-9A-Fa-f]|\\z' "${touched[@]}"
run_sweep "3 5.2+ stdlib" 'table\.(unpack|pack|move)|rawlen|coroutine\.isyieldable|math\.(type|tointeger)|utf8\.' "${touched[@]}"
run_sweep "4 // and bitwise" '[^:/]//|[[:alnum:]_)"] *(&|\||<<|>>) *[[:alnum:]_("]' "${touched[@]}"
run_sweep "5 sandbox/namespace" '\brequire[[:space:]]*\(|io\.popen|os\.(execute|exit)|\bminetest\.' "${touched[@]}"
...
if [[ -f tools/check_fresh_server.py ]]; then
