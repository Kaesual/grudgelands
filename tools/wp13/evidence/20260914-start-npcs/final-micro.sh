#!/usr/bin/env bash
# The single bounded final-byte process for the WP13 start-NPC increment: the
# four WP13 fixtures in one interpreter process, run once under LuaJIT and once
...
out="$repo/tools/wp13/evidence/20260914-start-npcs/final-micro"
cd "$repo"
mkdir -p "$out"
...
( cd "$repo" && sha256sum \
	tools/wp13/evidence/20260914-start-npcs/final-micro/micro-luajit.tsv \
	tools/wp13/evidence/20260914-start-npcs/final-micro/micro-puc51.tsv ) \
	>"$out/digests.txt"
cat "$out/digests.txt"
