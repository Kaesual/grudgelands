#!/usr/bin/env bash
# Static gates for the WP13 wave-3 polish lane, the same set every WP13
# increment runs (tools/wp13/evidence/20260914-character-visuals/static.sh):
...
#
# NOTE on sweep 1: the `::label::` half of it also matches a `::` inside a
# COMMENT, so the comments of every file below stay free of a doubled colon.
set -uo pipefail
export LC_ALL=C
