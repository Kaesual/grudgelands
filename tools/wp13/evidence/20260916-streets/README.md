...
| `mutation-review.py`, `mutation-review.txt` | the six mutations the independent review of 2026-09-16 wrote against this KAT — three of which it did not then catch — all red | `python3 tools/wp13/evidence/20260916-streets/mutation-review.py .` |
| `walkability.lua`, `walkability.txt` | **the headline result**: neighbouring road columns more than a node apart, over the whole capital as the seam writes it, on both trees and four seeds | `luajit tools/wp13/evidence/20260916-streets/walkability.lua . <seed>` |
| `gates.txt` | the three road gates that had to learn the viaduct rule, green: `kezamba_lots gates`, `lethariel_plots --bodies`, `lane_routes` | see `gates.sh` |
