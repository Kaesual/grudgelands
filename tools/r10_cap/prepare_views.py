...
for line in (scratch/'all-services.tsv').read_text().splitlines():
 ...
 if not key.endswith('/riding') and not key.startswith('highcourt/'):continue
 ...
 # Cutaway removes all cells at/above y=4 and front wall, but preserves floor,
 # furnishings and actual remaining wall cells. Full roof views are separate.
 if y<0 or y>=4 or (z<=-7 and y>0):continue
