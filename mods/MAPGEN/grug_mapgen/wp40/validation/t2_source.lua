...
local EXPECTED_POLICY_CHECKSUMS={
	...
	boundary_displacement="ed1cd5440d713e69d7dc913626490ae8c0af43e30a825ad9a81fcb6e13a60d2d",
...
local SOURCE_ARRAY_FIELDS={section_order=true,relief_profiles=true,
	...
	bay_edge_transitions=true,incident_bank_component_ids=true,
...
local EXPECTED_COUNTS = {
	...
	{"perimeter_spans",18},{"bay_bank_components",20},{"bay_edge_transitions",8},
	{"face_arcs",34},{"zone_faces",38},
...
local function validate_impl(source, vocabulary, canonical, raw_sha256)
	...
	local expected_sections={"critical_source_manifest","constants",
		...
		"bay_bank_components","bay_edge_transitions","face_arcs","zone_faces",
	...
	for _, family in ipairs({"relief_profiles","route_classes","water_classes",
			...
			"bay_bank_components","bay_edge_transitions",
	...
	local generic_policy_ids={
		...
		world_partition="face_partition_with_bay_capsule_water_v3",
	...
	local generic_policy_versions={boundary_displacement=3,world_partition=3}
	...
	if not dense(partition.classification_precedence) or
			...
			partition.classification_precedence[1]~=
				"raw_planned_Base_bay_water_in_strict_mainland_interior_or_own_mouth_aperture_equality_then_closure_Wing_water_in_strict_mainland_interior_then_single_pass_same_bay_notch_fill_in_strict_mainland_interior_only" or
			...
			partition.bay_mask_authority~=
				"raw_four_sample_round_capsule_union_then_two_literal_head_closure_wings_then_one_simultaneous_same_bay_notch_fill" or
			...
			partition.bay_notch_fill_policy_id~=
				"single_pass_same_bay_raw_mask_degree_one_notch_v1" or
			partition.bay_notch_fill_input~=
				"immutable_raw_final_planned_water_per_bay_equals_displaced_Base_union_closure_Wings_after_footprint_and_mouth_aperture_equality_precedence_before_any_notch_fill" or
			partition.bay_notch_fill_scope~=
				"for_each_bay_scan_only_its_finite_expanded_Base_and_Wing_envelope_and_require_P_and_all_eight_neighbours_strictly_inside_that_bay_final_mainland_footprint" or
			partition.bay_notch_fill_enumeration~=
				"semantic_domain_is_every_integer_P_in_the_deduplicated_envelope_the_complete_finite_candidate_superset_is_each_referenced_bay_raw_water_row_run_first_minus_one_and_finish_plus_one_because_every_three_of_four_cardinal_water_pattern_has_a_horizontal_water_neighbour_and_raw_dry_P_is_immediately_outside_that_run" or
			partition.bay_notch_fill_predicate~=
				"raw_dry_P_has_exactly_three_cardinal_and_all_four_diagonal_raw_water_neighbours_each_owned_only_by_the_same_referenced_bay_while_the_fourth_cardinal_neighbour_is_raw_dry" or
			partition.bay_notch_fill_application~=
				"evaluate_every_P_against_the_immutable_raw_mask_collect_all_unique_qualifying_Bay_P_pairs_then_union_them_simultaneously_once_never_iterate_or_feed_a_filled_cell_into_another_decision" or
			partition.bay_notch_fill_owner~=
				"each_filled_P_is_planned_water_of_its_unique_qualifying_bay_and_uses_that_bay_existing_exact_rational_owner_policy_no_neighbour_owner_copy_rank_snap_or_new_tie" or
			partition.bay_notch_fill_precedence~=
				"strict_mainland_interior_only_never_perimeter_or_mouth_aperture_equality_and_any_foreign_or_multiple_bay_water_contact_or_multiple_bay_qualification_rejects" or
			partition.bay_notch_fill_safe_arithmetic~=
				"Base_and_Wing_envelope_min_max_expansion_and_each_of_eight_neighbour_coordinates_use_checked_safe_integer_addition_or_subtraction_before_lookup_all_raw_mask_predicates_reuse_their_existing_exact_product_guards" or
			partition.bay_notch_fill_stage~=
				"extreme_scalar_records_and_their_4096_selector_identity_and_values_remain_upstream_unchanged_then_raw_mask_then_simultaneous_fill_produces_final_planned_water_then_R16_transition_selection_runs_once_then_bank_trace_face_partition_and_water_owner_resolution_use_that_same_final_mask" or
			partition.bay_notch_fill_payload~=
				"each_compiled_Bay_carries_policy_id_count_and_one_lexicographically_x_then_z_sorted_dense_array_of_exact_fill_x_z_columns_materialized_once_from_the_raw_mask_all_transition_bank_face_partition_and_owner_consumers_use_those_bytes_without_retracing_reclassification_or_face_inference" or
			...
			partition.bay_mouth_aperture_policy_id~=
				"maximal_contiguous_nonwrapping_half_open_exact_base_bay_perimeter_stations_v2" or
			...
			partition.bay_bank_candidate~=
				"final_classifier_dry_mainland_column_including_permitted_dry_perimeter_equality_with_four_neighbor_owned_by_same_final_planned_bay_mask_including_Base_Wings_and_single_pass_notch_fill" or
			...
			partition.bay_bank_reject~=
				"missing_K_malformed_nonadjacent_or_noncandidate_start_anchor_zero_terminal_reachable_successor_outgoing_bank_step_water_side_failure_reachability_frame_cap_stack_or_main_trace_bound_exhaustion_repeated_column_or_state_intra_or_inter_X_cross_foreign_bay_or_nonreferenced_water_contact_undeclared_endpoint_nonwing_column_inside_wedge_or_no_wedge_valid_joint_tail_pair_while_a_non_simple_or_zero_area_wedge_or_a_wedge_radius_above_five_excludes_only_that_pair" or
	...
	if boundary_policy.input_rule~=
			...
			boundary_policy.shared_boundary_clip_endpoint_rule~=
				"each_enumerated_interval_supplies_its_authored_from_and_to_integer_endpoint_for_attachment_or_ordinary_R18_obligation_probes_after_R18_fixes_one_interval_R19_exhaustively_resolves_the_closed_candidate_tuple_for_its_one_or_two_declared_bay_transition_endpoints" or
			...
			boundary_policy.shared_boundary_incidence_run_obligations~=
				"for_each_enumerated_interval_each_attachment_or_ordinary_junction_obligation_uses_that_intervals_authored_endpoint_and_each_transition_applies_the_existing_R16_direct_or_diagonal_resolution_only_to_that_intervals_authored_endpoint_an_interval_qualifies_only_if_both_ordered_R18_obligations_pass" or
			boundary_policy.shared_boundary_incidence_run_selection~=
				"select_the_exactly_one_interval_with_both_complete_ordered_R18_endpoint_obligations_zero_or_multiple_reject_no_longest_first_rank_or_numeric_index_tie_then_apply_R19_terminal_selection_inside_that_fixed_interval_without_reselecting_the_interval" or
			boundary_policy.shared_boundary_incidence_control_clip~=
				"keep_exactly_the_nonempty_unique_contiguous_authored_shifted_control_subsequence_whose_canonical_x_z_identities_lie_between_the_selected_final_endpoint_incidences_for_an_R19_edge_each_joint_one_or_two_transition_terminal_tuple_clips_the_combined_subsequence_applies_existing_R16_controls_and_is_probe_rerastered_by_the_sole_final_raster_pipeline_only_the_selected_complete_probe_bytes_are_materialized" or
			...
			boundary_policy.shared_boundary_incidence_excluded_dry~=
				"every_nonselected_dry_interval_and_every_dry_fragment_between_an_R19_terminal_and_its_selected_maximal_intervals_authored_endpoint_must_be_owned_exactly_once_by_a_final_Bank_or_dry_Face_and_by_no_final_land_edge_or_terminal_identity_else_reject" or
			...
			boundary_policy.shared_boundary_incidence_reject~=
				"missing_duplicate_or_mixed_obligation_zero_or_multiple_complete_intervals_zero_complete_transition_terminals_empty_nonunique_or_noncontiguous_ordinary_edge_interval_or_selected_result_control_subsequence_discarded_required_terminal_foreign_gap_or_excluded_fragment_missing_duplicate_or_land_edge_owned_rejects_without_fallback_while_an_empty_or_noncontiguous_per_tuple_combined_clip_fails_only_that_tuple" or
			...
			boundary_policy.bay_edge_transition_direct~=
				"E_is_each_eligible_authored_station_incidence_enumerated_toward_the_interior_from_the_declared_from_or_to_side_of_the_current_selected_maximal_final_dry_interval_if_E_is_a_same_bay_final_dry_boundary_candidate_R16_resolves_E_unchanged_for_that_candidate_probe" or
			boundary_policy.bay_edge_transition_diagonal_precondition~=
				"otherwise_E_must_be_final_strict_dry_and_have_no_cardinal_same_bay_or_foreign_water_neighbor_W_is_the_immediately_adjacent_R7_station_from_E_toward_the_declared_edge_endpoint_and_is_excluded_when_that_candidate_clip_is_probed_E_to_W_must_be_exactly_diagonal_and_W_must_be_final_planned_water_owned_only_by_the_referenced_bay_after_Base_Wings_and_single_pass_notch_fill" or
			...
			boundary_policy.bay_edge_transition_application~=
				"R16_resolve_each_eligible_terminal_incidence_inside_the_R18_selected_maximal_interval_then_for_each_closed_one_or_two_endpoint_candidate_tuple_clip_the_combined_retained_control_subsequence_and_probe_the_existing_sole_final_edge_raster_R19_selects_the_unique_complete_tuple_and_materializes_its_already_resolved_edge_bytes_once_no_post_raster_append_reraster_or_snap" or
			...
			boundary_policy.bay_edge_transition_scalar_scope~=
				"E_W_any_derived_elbow_and_the_R19_terminal_are_resolved_only_after_record_scalar_materialization_R16_R17_R18_and_R19_never_enter_or_change_extreme_scalar_record_identity_or_value_no_pre_correction_C1_pool_is_promotable_and_the_sole_promotable_complete_4096_pool_and_selected_winners_are_generated_only_against_later_approved_immutable_post_R19_Source_and_compiler_pins" or
			...
			boundary_policy.bay_edge_transition_terminal_policy_id~=
				"joint_bank_incidence_transition_terminal_v1" or
			boundary_policy.bay_edge_transition_terminal_candidates~=
				"for_each_declared_transition_endpoint_of_the_fixed_R18_selected_maximal_dry_interval_every_station_incidence_except_the_opposite_interval_endpoint_is_eligible_exactly_when_it_has_the_immediately_adjacent_in_interval_station_away_from_the_declared_endpoint_run_R16_for_every_eligible_incidence_collect_every_successful_resolved_candidate_then_exhaustively_enumerate_the_cartesian_tuple_of_one_candidate_per_declared_transition_endpoint_with_no_other_pruning_first_success_scan_or_private_order" or
			boundary_policy.bay_edge_transition_terminal_dependency~=
				"an_edge_has_exactly_one_or_two_declared_transition_endpoints_each_transitions_two_declared_incident_Banks_are_exactly_one_start_and_one_end_and_each_other_terminal_is_aperture_or_Wing_derived_never_an_edge_transition_so_each_joint_edge_tuple_probe_is_finite_and_acyclic" or
			boundary_policy.bay_edge_transition_terminal_complete~=
				"for_each_candidate_tuple_require_a_nonempty_combined_contiguous_R7_control_subsequence_candidate_clip_it_apply_all_R16_E_then_optional_T_controls_and_probe_the_sole_final_edge_raster_require_unique_eight_connected_no_X_cross_final_dry_record_envelope_valid_probe_bytes_each_resolved_terminal_as_its_declared_probe_edge_endpoint_bind_previous_to_the_immediately_adjacent_probe_final_edge_station_away_then_require_both_incident_Banks_of_every_transition_to_complete_under_the_unchanged_R11_candidate_water_side_DFS_repeat_and_X_cross_rules" or
			boundary_policy.bay_edge_transition_terminal_selection~=
				"the_least_complete_joint_edge_tuple_across_the_full_finite_cartesian_set_is_selected_under_the_declared_total_order_total_retreat_from_the_declared_endpoints_then_maximum_per_endpoint_retreat_then_elbow_terminal_count_then_sorted_resolved_terminal_set_then_sorted_previous_set_then_probe_bytes_under_canonical_orientation_two_distinct_tuples_with_the_same_terminal_previous_and_probe_edge_byte_identity_are_duplicate_authority_and_reject_zero_complete_tuples_reject_enumeration_iteration_order_first_longest_nearest_scan_backstep_side_flip_and_private_tie_remain_forbidden_the_declared_order_is_selection_among_complete_tuples_not_pruning" or
			boundary_policy.bay_edge_transition_terminal_application~=
				"promote_only_the_selected_joint_tuple_probe_final_edge_bytes_then_materialize_the_shared_edge_and_all_incident_Banks_once_excluded_dry_columns_keep_owner_validation_and_no_unselected_R16_resolution_probe_edge_or_probe_Bank_path_is_serialized" or
			boundary_policy.bay_edge_transition_terminal_bounds~=
				"per_endpoint_at_most_selected_interval_station_count_minus_one_eligible_incidences_and_resolved_candidates_each_edge_has_at_most_two_transition_endpoints_and_at_most_the_checked_product_of_their_candidate_counts_joint_tuples_each_tuple_runs_at_most_four_existing_R11_traces_with_existing_per_Bay_8N_frame_N_stack_and_N_minus_1_main_step_caps_all_counts_products_and_neighbor_arithmetic_checked_safe_integer" or
			boundary_policy.bay_edge_transition_terminal_reversal~=
				"authored_edge_reversal_swaps_endpoint_direction_and_the_one_or_two_endpoint_candidate_tuple_orientation_but_resolves_the_same_world_terminals_and_probe_previous_stations_probe_final_edge_bytes_reverse_exactly_and_Bank_consumers_reverse_only_finished_bytes" or
			boundary_policy.bay_edge_transition_terminal_reject~=
				"candidate_outside_interval_malformed_or_missing_adjacent_away_station_duplicate_resolved_tuple_identity_nonadjacent_or_missing_probe_anchor_edge_overlap_other_than_terminal_unreachable_incident_Bank_zero_complete_joint_tuples_changed_R7_scalar_identity_excluded_fragment_owner_failure_or_face_and_Whole_topology_failure_rejects_without_scan_snap_tail_connector_or_seed_fallback" or
	...
	do
		local expected_transitions={
			{"bay_edge_transition:land_001:to","bay_elandor_west","land_001","to",
				"bay_bank:elandor_west:hearthpine","bay_bank:elandor_west:copperfell"},
			{"bay_edge_transition:land_004:from","bay_elandor_west","land_004","from",
				"bay_bank:elandor_west:goldmead","bay_bank:elandor_west:dawnmere"},
			{"bay_edge_transition:land_004:to","bay_elandor_east","land_004","to",
				"bay_bank:elandor_east:dawnmere","bay_bank:elandor_east:goldmead"},
			{"bay_edge_transition:land_007:from","bay_elandor_east","land_007","from",
				"bay_bank:elandor_east:starbough","bay_bank:elandor_east:silverleaf"},
			{"bay_edge_transition:land_010:to","bay_kragmar_west","land_010","to",
				"bay_bank:kragmar_west:stillgrave","bay_bank:kragmar_west:mournfen"},
			{"bay_edge_transition:land_013:from","bay_kragmar_west","land_013","from",
				"bay_bank:kragmar_west:redtusk","bay_bank:kragmar_west:sunscar"},
			{"bay_edge_transition:land_013:to","bay_kragmar_east","land_013","to",
				"bay_bank:kragmar_east:sunscar","bay_bank:kragmar_east:redtusk"},
			{"bay_edge_transition:land_016:from","bay_kragmar_east","land_016","from",
				"bay_bank:kragmar_east:raincall","bay_bank:kragmar_east:kapok"},
		}
		local projected,projected_sides={},{}
		for bank_index=1,#source.bay_bank_components do
			...
			for terminal_index=1,2 do
				...
				if terminal.kind=="land_edge_transition" then
					...
					projected[key]=projected[key] or {}
					projected_sides[key]=projected_sides[key] or {}
					projected[key][#projected[key]+1]=bank.id
					projected_sides[key][#projected_sides[key]+1]=
						terminal_index==1 and "start" or "end"
				end
			end
		end
		local bay_ids={}
		...
		for transition_index=1,#source.bay_edge_transitions do
			...
			local path="bay_edge_transitions["..transition_index.."]"
			local fields_ok,fields_failure=closed_fields(row,path,
				{"id","bay_id","edge_id","edge_endpoint","incident_bank_component_ids",
					"resolution_policy_id","terminal_selection_policy_id",
					"candidate_tie_rule","numeric_id"},
				"bay_edge_transition_fields")
			if not fields_ok then return nil,fields_failure end
			local incident=row.incident_bank_component_ids
			if not dense(incident) or #incident~=2 then
				return diag("bay_edge_transition_incidence_fields",row.id,
					"two ordered bank component IDs",type(incident)=="table" and #incident or type(incident))
			end
			local key=row.edge_id..":"..row.edge_endpoint
			if seen[key] then
				return diag("bay_edge_transition_projection",row.id,"unique edge endpoint",key)
			end
			seen[key]=true
			transition_by_endpoint[key]=row.id
			transition_edge_count[row.edge_id]=(transition_edge_count[row.edge_id] or 0)+1
			...
			local sides=projected_sides[key]
			if not actual or #actual~=2 or not sides or #sides~=2 then
				return diag("bay_edge_transition_projection",row.id,
					"exactly two projected bank terminals",actual and #actual or 0)
			end
			for incident_index=1,2 do
				local bank=bank_by_id[incident[incident_index]]
				if incident[incident_index]~=actual[incident_index] or
						not bank or bank.bay_id~=row.bay_id then
					return diag("bay_edge_transition_incidence",row.id,
						"source-ordered same-Bay bank projection",incident[incident_index])
				end
				local matching_count=0
				local other_terminal
				for terminal_index=1,2 do
					local terminal=terminal_index==1 and bank.start_terminal or bank.end_terminal
					if terminal.kind=="land_edge_transition" and
							terminal.edge_id==row.edge_id and
							terminal.edge_endpoint==row.edge_endpoint then
						matching_count=matching_count+1
						other_terminal=terminal_index==1 and bank.end_terminal or bank.start_terminal
					end
				end
				if matching_count~=1 or not other_terminal or
						(other_terminal.kind~="aperture_dry" and
						other_terminal.kind~="wing_junction_tail_side") then
					return diag("bay_edge_transition_terminal_dependency",bank.id,
						"one matching transition terminal and one fixed aperture or Wing terminal",
						matching_count..":"..tostring(other_terminal and other_terminal.kind))
				end
			end
			if sides[1]==sides[2] or
					(sides[1]~="start" and sides[2]~="start") or
					(sides[1]~="end" and sides[2]~="end") then
				return diag("bay_edge_transition_terminal_sides",row.id,
					"one start and one end terminal",sides[1]..":"..sides[2])
			end
		end
		for edge_id,count in pairs(transition_edge_count) do
			if count<1 or count>2 then
				return diag("bay_edge_transition_terminal_dependency",edge_id,
					"one or two transition endpoints per edge",count)
			end
		end
		...
		for transition_index=1,#source.bay_edge_transitions do
			...
			if row.id~=expected[1] or row.bay_id~=expected[2] or
					...
					row.resolution_policy_id~="direct_candidate_or_same_bay_diagonal_elbow_v1" or
					row.terminal_selection_policy_id~="joint_bank_incidence_transition_terminal_v1" or
					row.candidate_tie_rule~="lexicographically_least_x_then_z" or
					row.numeric_id~=transition_index then
				return diag("bay_edge_transition_contract",row.id,
	...
	for aperture_index=1,#source.bay_mouth_apertures do
		...
		if row.id~=expected[1] or row.bay_id~=expected[2] or row.perimeter_id~=expected[3] or
				...
				row.policy_id~="maximal_contiguous_nonwrapping_half_open_exact_base_bay_perimeter_stations_v2" or
...
validator.EXPECTED_BOUNDARY_DISPLACEMENT_CHECKSUM =
	EXPECTED_POLICY_CHECKSUMS.boundary_displacement
validator.EXPECTED_WORLD_PARTITION_CHECKSUM =
	EXPECTED_POLICY_CHECKSUMS.world_partition
