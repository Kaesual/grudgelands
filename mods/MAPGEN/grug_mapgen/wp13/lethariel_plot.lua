-- One Lethariel district plot, built from a roster row.
--
-- This is `wp13/highcourt_plot.lua`'s job for the elf capital. It is a
-- SEPARATE builder and not a parameter of that one for two reasons, and both
-- are ownership rather than taste: `highcourt_plot.lua` is the pilot capital's
-- committed builder, its 52 plots are measured against it and this lane may
-- not move a cell of them; and every line of it that names a palette names
-- `palettes.new("human")`, so "the same builder for another race" would have
-- been a rewrite of it in place. What is shared is the RULE, not the code, and
-- the rule is the capitals contract's section 2.1.
--
...
-- envelope -- step 3 for the elf terraced grove -- so two plots sixty nodes
-- apart stand at two different heights. The contract settles that with a
-- REFERENCE COLUMN: each plot is authored around its own origin with y = 0 the
-- ground course, and at settle time the settlement config asks the pure final
-- height of one named column once, caches it, and projects the plot's cells
-- from there. Two halves make that survive a terrace edge under the plot:
--
--   * a FOUNDATION SKIRT: the perimeter carried down to y = -6, so a plot
--     whose downhill corner hangs over the step still stands on masonry;
...
-- block of both would cost a plot thousands of cells of buried stone.
--
...
-- the same ground course, skirt, clear and sockets, plus whatever its own
-- `decorate` plants on it. That is what a grove, a garden, a pond or a
...
local function loader(directory)
	...
	-- The plot's own ground area: the part's extent grown by the margin on
	-- every side and then clamped into the lot's own reach, so the kerb, the
	-- doorstep path and the lanterns stand on the plot and not beside it.
	...
	-- The socket shapes a roster may publish through `plot.extra_sockets`,
	-- spelled once so four rosters spell them the same way. They are the
	-- sockets contract's own shapes and nothing else.
	...
	-- `spawn = false` is the whole of it. NO TAG, deliberately: a tag is what
	-- an idle NPC's spoken line reads off, and nobody stands here to say
	-- anything.
	...
	-- workplace, always staffed, always static, naming the ACTIVITY it does
	-- and facing the feature that activity works within three nodes.
	-- `y` is the feet course and defaults to 1; a socket ON a bench passes 2,
	-- because a seat is a walkable node and the resident sits on top of it.
	...
	-- A plain idle spot a roster wants somewhere its plot alone knows about.
	function M.idle(id, x, z, face, tags, y)
		return {id = "idle_" .. id, role = "idle", x = x, y = y or 1, z = z,
			face = face or 0, tags = tags}
	end

	-- A YARD: the open-ground plot. Its own half-extent, which is the lot's
	...
	local function yard_area(plot)
		...
		if type(half_x) ~= "number" or type(half_z) ~= "number" or
			...
			error("wp13 lethariel plot: the yard " .. tostring(plot.id) ..
				" has no extent", 0)
		end
	...
	--
	-- The row: `id`, `module` ("capitals", "buildings" or "elf"), `make` (the
	-- generator), `spec` (its arguments), and optionally `turns`, `margin`,
	-- `roof` ("pale"), `garden`, `order` (its waypoint in the district loop),
	-- `decorate`, `extra_sockets` and `socket_overrides`.
	function M.build(plot, context)
		context = context or {}
		local watch = plot.patrol_group or context.patrol_group
		local palette = handles.elf
		local roof_palette
		if plot.roof == "pale" then roof_palette = handles.pale end
		if plot.handle == "pale" then palette = handles.pale end
		local spec = {}
		...
		if plot.yard then
			...
			if type(generator) ~= "function" then
				error("wp13 lethariel plot: no generator " ..
		...
		if plot.yard then
			...
			-- its own kerb, and the two lanterns every plot carries.
			for z = z0, z0 + 1 do
				for x = -1, 1 do
			...
			-- own apron out to the street edge.
			...
			-- 5. Two lanterns on the street corners of the plot, and a bench
			-- beside the door. LANTERN PILLARS and not plain lamp posts: the
			-- contract's elf row is "lantern-lit walks", the elf palette binds
			-- `light_beacon`, and `dressing.lantern_pillar` is the piece that
			-- uses it. It is three by three, so it stands one node further in
			-- than a post would.
		...
		-- silverwood standard at each corner, planters either side of the
		-- door and the carrier's crates by the gate.
		if plot.garden then
			for _, spot in ipairs({{x0 + 2, z0 + 2}, {x1 - 2, z0 + 2},
					{x0 + 2, z1 - 2}, {x1 - 2, z1 - 2}}) do
				dressing.columnar(buf, palette, spot[1], spot[2], 6)
			end
		...
		-- 5c. Whatever this plot alone wants on its own ground. It is handed
		-- the buffer, the palette and the plot's ground rectangle, and it runs
		...
		local area = {x0 = x0, z0 = z0, x1 = x1, z1 = z1, ox = ox, oz = oz,
			...
			dressing = dressing, parts = parts, elf = elf,
			handles = handles, merged = merged}
		...
		-- socket of their own, and a district of nine plots with four
		-- inhabited ones is not a district anybody lives in.
		...
		services.decorate("lethariel", plot.id, buf, palette, sockets, area)
		...
		local hanging = palette.maybe("light_hanging")
		...
		return {
			...
			-- compares `blueprint.schema` with the profile's own, so nine
			-- plots behind one string would need nine profiles that all claim
			-- to be the same blueprint.
			...
			-- y = 0. It is the plot origin, that is the middle of the plot: a
			-- plot levels to the ground under its own centre, never to a
			-- corner, or half of every plot on a slope would stand a terrace
			-- step too high.
			reference = {x = 0, z = 0},
			-- The airspace this plot actually cleared. `bounds.max.y` is not
			-- that number: a lantern pillar or a silverwood standard written
			-- after the clear reaches above it, so a legality predicate
			-- reading the bounds would credit the plot with headroom it never
			-- cut.
			clear_to = clear_to,
			landmarks = {
		...
		local function entries(list, into, yard_reaches)
			for index, plot in ipairs(list or {}) do
				...
				if yard_reaches then
					...
					if reach == nil then
						error("wp13 lethariel plot: the fill row " ..
							tostring(plot.id) .. " has no lot", 0)
					end
					plot.reach = reach
