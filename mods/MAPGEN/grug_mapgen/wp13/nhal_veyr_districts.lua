-- Nhal Veyr's four districts, and where this world puts them.
--
-- The four rosters are `nhal_veyr_district.lua` (the ossuary market and
-- trades), `nhal_veyr_district_martial.lua` (the bone watch),
-- `nhal_veyr_district_lore.lua` (the vigil) and `nhal_veyr_district_homes.lua`
-- (the kept houses); the four quadrants' lot grids and the seeded permutation
-- are `nhal_veyr_quadrants.lua`. This file is the one place the two meet, and
-- it is what the WP40 capital source asks for its plot list.
--
...
-- it. The fill plots travel with their DISTRICT, which is why they are here
-- and not a fifth roster of their own: the vigil's ruin close belongs beside
-- the vigil wherever the seed puts it. The order does not depend on the seed,
-- only the offsets do, which is what keeps the manifest's field order and
...
local function loader(directory)
	local quadrants = dofile(directory .. "/nhal_veyr_quadrants.lua")()
	local market = dofile(directory .. "/nhal_veyr_district.lua")(directory)
	local martial =
		dofile(directory .. "/nhal_veyr_district_martial.lua")(directory)
	local lore = dofile(directory .. "/nhal_veyr_district_lore.lua")(directory)
	local homes = dofile(directory .. "/nhal_veyr_district_homes.lua")(directory)
	...
	-- `nhal_veyr_quadrants.ROLES` names them in and the order the permutation
	-- is read against.
	...
	do
		...
		if #M.districts ~= #roles then
			error("wp13 nhal veyr: district roster differs", 0)
		end
		for index = 1, #roles do
			if M.districts[index].role ~= roles[index] then
				error("wp13 nhal veyr: district " .. index .. " is not " ..
					roles[index], 0)
			end
			if #M.districts[index].plots ~= #quadrants.AUTHORED then
				error("wp13 nhal veyr: " .. roles[index] ..
	...
	--
	-- `options` is the quadrant seam of `nhal_veyr_quadrants.assign`:
