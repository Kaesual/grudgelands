...
--
-- It writes strictly inside grug_core's probe window, which is the volume
-- request_camp_platform emerged, and it stops below the first y whose
-- footprint still reads `ignore`: writing into blocks the mapgen has not
-- generated yet would blit blank blocks into the map. The clearing above that
-- line is not lost — build_camp covers the whole camp volume, not just the
-- chunk holding the anchor, so the mapchunks above clear their own slice when
-- they are generated.
...
function grug_core.ensure_camp_platform_built(race_id)
	...
	local y2 = math.min(y + CLEAR_HEIGHT, grug_core.CAMP_PROBE_TOP)
	...
	vm:set_data(data)
	-- Same reason as the mapchunk pass below: the clearing opens a shaft that
	-- can break into standing water, and a main-env write does not queue the
	-- liquid/lighting updates finishBlockMake would.
	vm:update_liquids()
	vm:calc_lighting()
	vm:write_to_map()
	core.log("action", ("[grug_mapgen] %s capital platform built from the map " ..
		"at %d,%d,%d (mapgen never saw the decided height)")
