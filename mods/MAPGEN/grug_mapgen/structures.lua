...
local WALL_X = grug_core.WORLD_HALF_WIDTH_X
...
-- The result is persisted via grug_core (decided once, never shifts).
...
core.register_on_generated(function(minp, maxp, blockseed)
	...
	local camps = {}
	for id, def in pairs(grug_core.factions) do
		local s = def.spawn
		if maxp.x >= s.x - CAMP_HALF and minp.x <= s.x + CAMP_HALF and
				maxp.z >= s.z - CAMP_HALF and minp.z <= s.z + CAMP_HALF then
			local y = grug_core.get_camp_platform_y(id)
			if not y then
				y = decide_platform_y(s, minp, maxp)
				if y then
					grug_core.set_camp_platform_y(id, y)
					core.log("action", ("[grug_mapgen] %s camp platform " ..
