...
-- anyway), the floor is below the deepest sea bed a capital column can have.
local PROBE_TOP = 120
local PROBE_BOTTOM = -16
...
-- median of each column's topmost non-liquid walkable node, i.e. exactly the
-- quantity the heightmap median approximates. It therefore CANNOT be
-- chunk-order-dependent: it is a pure function of the generated terrain, and
-- it only ever runs on a footprint that is fully emerged (see
...
-- column scan below stays a plain array walk. Both "air" and "ignore" carry
-- walkable = false in builtin (register.lua), so the one test covers the
-- un-emerged case too.
local function is_ground_cid(cid)
	...
	if v == nil then
		...
		v = def ~= nil and def.walkable ~= false and
			core.get_item_group(name, "liquid") == 0
		ground_cid[cid] = v
	end
	return v
end
...
local function probe_platform_y(capital)
	...
	-- Reused buffer: a fresh get_data() table per call is pure garbage.
...
-- Makes sure this capital's platform height gets DECIDED, and returns
-- nothing: the answer arrives asynchronously in mod storage. Idempotent and
-- at most one emerge per race per session.
--
...
-- pass, which decides and persists. If that still leaves the platform
...
-- decides from the finished map.
local platform_requested = {}

function grug_core.request_camp_platform(race_id)
	if platform_requested[race_id] then
		return
	end
	local capital = grug_core.capitals[race_id]
	if not capital then
	...
	platform_requested[race_id] = true
	if grug_core.get_camp_platform_y(race_id) then
		return -- already decided (stored, or the engine answered just now)
	end
	local r = grug_core.CAMP_SAMPLE_RADIUS
	core.emerge_area(
		...
		function(_, _, remaining)
			...
			if grug_core.get_camp_platform_y(race_id) then
				return -- grug_mapgen decided it while generating: it wins
			end
			local y = probe_platform_y(capital)
			if y then
				grug_core.set_camp_platform_y(race_id, y)
			else
				core.log("warning", ("[grug_core] %s capital platform stays " ..
					"undecided: no ground at %d,%d between y=%d and y=%d")
					:format(race_id, capital.x, capital.z, PROBE_BOTTOM,
						PROBE_TOP))
			end
		end)
end
...
-- world" problem one level down. Six emerges, once per world (the requests
-- are no-ops the moment a height is stored), and deliberately late and
