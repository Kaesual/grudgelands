...
-- GRUG PATCH: fresh-server mode - `replace_old_owner_information()` copied the
-- pre-"owner" `doors_owner` node meta key forward and is removed together with
-- its four call sites. A fresh world only ever writes "owner".
...
function doors.door_toggle(pos, node, clicker)
	...
	-- GRUG PATCH: old `doors_owner` meta migration call removed (see top).
...
function doors.register(name, def)
	...
	-- GRUG PATCH: fresh-server mode - the per-door ":doors:replace_<name>" LBM is
	-- removed. It converted the old two-node "<name>_b_1"/"_b_2" door format
	-- (plus its "right" meta flag) into the current _a/_b/_c/_d nodes; those
	-- names cannot occur on a fresh world.
	...
	-- GRUG PATCH: `on_rotate` removed - it is the `screwdriver` hook and
	-- Grudgelands does not ship that mod (upstream optional_depends dropped in
	-- mod.conf). Doors were never rotatable anyway; the handler only said "no".
	...
	if def.protected then
		...
		def.on_skeleton_key_use = function(pos, player, newsecret)
			-- GRUG PATCH: old `doors_owner` meta migration call removed.
			local meta = minetest.get_meta(pos)
...
doors.register("door_steel", {
		...
		protected = true,
		-- GRUG PATCH: no `level` group - grug_materials retired the engine level
		-- gate and its startup audit hard-fails on any node with level > 0
		-- (grug_materials/audit.lua, "node retains non-zero level group").
		groups = {node = 1, cracky = 1},
		sounds = default.node_sound_metal_defaults(),
		sound_open = "doors_steel_door_open",
		sound_close = "doors_steel_door_close",
		gain_open = 0.2,
		gain_close = 0.2,
		-- GRUG PATCH: no `recipe` - it consumed `default:steel_ingot`, which
		-- grug_materials/content_curation.lua retires and unregisters (the
		-- canonical bar is grug_materials:iron_bar). The door stays registered
		-- and placeable.
})
...
-- GRUG PATCH: `doors.register_door()` removed - it was the deprecated
-- pre-`doors.register` API shim ("Capture mods using the old API as best as
-- possible"), including its `only_placer_can_open` translation and fallback
-- tiledef. Every shipped caller uses `doors.register` / `doors.register_trapdoor`.
...
-- GRUG PATCH: the Steel Trapdoor recipe is removed - it consumed
-- `default:steel_ingot`, retired and unregistered by
-- grug_materials/content_curation.lua. The trapdoor stays registered.
