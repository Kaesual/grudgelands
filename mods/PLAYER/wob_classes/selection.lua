-- Character creation flow: faction (wob_factions) -> race -> class, all
-- mandatory, all final. Closing a dialog without choosing re-opens it.

local RACE_FORM = "wob_classes:race"
local CLASS_FORM = "wob_classes:class"
...
local function show_race_selection(player)
	local faction_id = wob_factions.get_faction(player)
	if not faction_id then
		return
	end
	local options = {}
	for _, id in ipairs(wob_classes.race_ids[faction_id] or {}) do
		table.insert(options, wob_classes.registered_races[id])
	end
	core.show_formspec(player:get_player_name(), RACE_FORM,
		options_formspec("Choose your race!",
			"Where in the " .. wob_core.factions[faction_id].name ..
			" lands do you come from? This decision is final.", options))
end

local function show_class_selection(player)
	local options = {}
	for _, id in ipairs(wob_classes.class_ids) do
		table.insert(options, wob_classes.registered_classes[id])
	end
...
local function continue_creation(player)
	if not wob_factions.get_faction(player) then
		return -- wob_factions owns this step
	end
	if not wob_classes.get_race(player) then
		show_race_selection(player)
	elseif not wob_classes.get_class(player) then
...
core.register_on_player_receive_fields(function(player, formname, fields)
	if formname == RACE_FORM then
		if wob_classes.get_race(player) or
				not wob_classes.set_race(player, chosen_id(fields) or "") then
			reopen_later(player)
			return true
		end
	...
	elseif formname == CLASS_FORM then
		if wob_classes.get_class(player) or
				not wob_classes.set_class(player, chosen_id(fields) or "") then
			reopen_later(player)
			return true
		end
...
wob_factions.register_on_faction_chosen(function(player, faction_id)
...
core.register_chatcommand("char", {
	...
	func = function(name)
		...
		local a = wob_classes.get_attributes(player)
		return true, ("%s %s %s, level %d | Str %d Int %d Dex %d | " ..
			...
			wob_xp.get_level(player),
			a.str, a.int, a.dex,
			player:get_hp(), wob_classes.get_max_hp(player),
			wob_classes.get_max_mana(player),
