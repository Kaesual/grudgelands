...
-- the wob_xp level pipeline. Ability/mana HUDs and the damage pipeline that
-- consumes melee/spell/crit values arrive with WP4; the accessors below are
-- their single source of truth.

local BASE_ATTR = 10

function wob_classes.get_attributes(player)
...
function wob_classes.get_max_hp(player)
	return 20 + 2 * (wob_xp.get_level(player) - 1)
		+ wob_classes.get_attributes(player).str
end

-- 0 for classes that use rage (or no class yet).
function wob_classes.get_max_mana(player)
	local def = wob_classes.get_class_def(player)
	if not def or def.resource ~= "mana" then
		return 0
	end
	return 10 + 2 * wob_classes.get_attributes(player).int
end

-- Flat bonus added to weapon damage.
function wob_classes.get_melee_bonus(player)
	return math.floor(wob_classes.get_attributes(player).str / 10)
end

-- Flat bonus added to ability base values.
function wob_classes.get_spell_power_bonus(player)
	return math.floor(wob_classes.get_attributes(player).int / 10)
end

-- Chances in 0..1; flat caps, no diminishing returns (combat_stats.md §2).
function wob_classes.get_crit_chance(player)
	return math.min(0.30, 0.05 + 0.001 * wob_classes.get_attributes(player).dex)
end

function wob_classes.get_dodge_chance(player)
	return math.min(0.30, 0.001 * wob_classes.get_attributes(player).dex)
end
...
function wob_classes.apply_stats(player, heal_gain)
	local max_hp = wob_classes.get_max_hp(player)
