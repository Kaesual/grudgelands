...
local function content(player, context)
	...
	local fs = {
		("checkbox[0.15,0.08;grug_party_invites;Allow invitations;%s]")
			:format(grug_parties.invitations_enabled(player) and "true" or "false"),
		("checkbox[3.45,0.08;grug_party_hud;Party HUD;%s]")
			:format(grug_parties.hud_enabled(player) and "true" or "false"),
		"label[0.15,0.65;Invite a player]",
		"field[0.15,1.05;3.25,0.65;grug_party_invite_name;;]",
		"button[3.50,0.92;1.25,0.65;grug_party_invite;Invite]",
		"label[5.05,0.65;Pending invitations]",
	}
	local invites = {}
	...
	fs[#fs + 1] = ("textlist[5.05,1.05;5.15,1.45;grug_party_pending;%s;%d;false]")
		:format(table.concat(invites, ","), selected_index)
	fs[#fs + 1] = "button[5.05,2.58;1.35,0.6;grug_party_accept;Accept]"
	fs[#fs + 1] = "button[6.55,2.58;1.35,0.6;grug_party_decline;Decline]"
	if view then
		fs[#fs + 1] = "label[0.15,1.85;Current party]"
		local members = {}
		...
		fs[#fs + 1] = ("textlist[0.15,2.25;4.60,3.55;grug_party_members;%s;%d;false]")
		...
		if view.leader == own_name and selected.name ~= own_name then
			fs[#fs + 1] = "button[0.15,5.95;1.45,0.65;grug_party_kick;Kick]"
			fs[#fs + 1] = "button[1.75,5.95;2.15,0.65;grug_party_transfer;Make leader]"
		end
		fs[#fs + 1] = "button[4.05,5.95;1.35,0.65;grug_party_leave;Leave]"
	else
		fs[#fs + 1] = "textarea[0.15,2.10;4.60,1.15;;;You are not in a party. Invite a same-faction player to form one.]"
	end
	if context.grug_party_notice then
		fs[#fs + 1] = "textarea[5.05,3.45;5.15,1.2;;;" .. esc(context.grug_party_notice) .. "]"
	end
	return table.concat(fs)
end

sfinv.register_page(PAGE, {title = "Group", get = function(_, player, context)
	...
	local ok, message
	if fields.grug_party_invite then ok, message = grug_parties.invite(player, fields.grug_party_invite_name or "")
