...
wob_inventory.BAG_COUNT = 4

function wob_inventory.bag_list(i)
	return "wob_bag" .. i
end

function wob_inventory.content_list(i)
	return "wob_bag" .. i .. "_content"
end

local function bag_slot_index(listname)
	return tonumber(listname:match("^wob_bag(%d)$"))
end

local function content_index(listname)
	return tonumber(listname:match("^wob_bag(%d)_content$"))
end

function wob_inventory.bag_slots_of(stack)
...
core.register_craftitem("wob_inventory:bag_small", {
	description = "Small Bag (8 slots)",
	inventory_image = "wob_inventory_bag_small.png",
	stack_max = 1,
	groups = {bagslots = 8},
})

core.register_craftitem("wob_inventory:bag_medium", {
	description = "Medium Bag (16 slots)",
	inventory_image = "wob_inventory_bag_medium.png",
	stack_max = 1,
	groups = {bagslots = 16},
})

core.register_craftitem("wob_inventory:bag_large", {
	description = "Large Bag (24 slots)",
	inventory_image = "wob_inventory_bag_large.png",
	stack_max = 1,
	groups = {bagslots = 24},
})
...
core.register_on_joinplayer(function(player)
	...
	for i = 1, wob_inventory.BAG_COUNT do
		inv:set_size(wob_inventory.bag_list(i), 1)
		local bag = inv:get_stack(wob_inventory.bag_list(i), 1)
		inv:set_size(wob_inventory.content_list(i),
			wob_inventory.bag_slots_of(bag))
	end
end)
...
core.register_allow_player_inventory_action(function(player, action, inventory, info)
	...
	if from_bag and not inventory:is_empty(wob_inventory.content_list(from_bag)) then
		return 0
	end
	...
	if to_list then
		...
		if to_bag then
			if wob_inventory.bag_slots_of(stack) == 0 then
			...
			if not current:is_empty() and
					not inventory:is_empty(wob_inventory.content_list(to_bag)) then
				return 0
			end
			return 1
		end
		if content_index(to_list) and wob_inventory.bag_slots_of(stack) > 0 then
...
core.register_on_player_inventory_action(function(player, action, inventory, info)
	...
	for i in pairs(touched_bags) do
		local bag = inventory:get_stack(wob_inventory.bag_list(i), 1)
		inventory:set_size(wob_inventory.content_list(i),
			wob_inventory.bag_slots_of(bag))
		refresh = true
	end
	if refresh then
