...
--
-- Nametag (combat_stats.md section 6): neutral white, per viewer through the
-- observer-managed child entity in grug_core/tag_carrier.lua. Con colors stay
-- in the HUD target frame.
--
-- The shared carrier module owns the only player-position snapshot. Settlement
-- work animation also asks this nearest-player view, so keep the old public
-- seam without maintaining a second globalstep.
local nearest_player_d2 = grug_core.nearest_tag_player_d2
...
--
-- `_grug_tag` is the desired text. Health, telegraph and tier changes write it
-- to the carrier immediately; only players currently in its observer set get
-- that property update, and a later observer receives the carrier's current
-- property when the object is first sent.
local function update_tag(self)
	...
	self._grug_tag = text
	grug_core.set_tag_carrier_text(grug_mobs.ensure_tag_carrier(self), text)
end
...
-- of the settlement work animations
