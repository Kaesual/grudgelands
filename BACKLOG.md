...
| WP11 | Skill trees: 2 trees × 2 chains × 8 talents = 30 ranks per tree (60 per class), 1 point per 2 levels (30 at level 60 = one whole tree), 5 numeric talents + 2 keystones + 1 capstone per tree; a keystone is a NEW active main skill, a capstone is the tree's top talent and may be a skill or a strong effect (e.g. Priest: Renew is the Mercy keystone); 15 new ability registrations for the three shipped classes (11 keystones + 4 skill capstones); respec for money **in the talent UI, no class trainer** | open (spec: `docs/design/skill_trees.md` PROPOSAL rev 2 + `progression.md` §2; user rulings 2026-09-16; open decisions in `skill_trees.md` §6, §6.2 asks whether the capstones split into a follow-up WP) | WP3, WP4 |
| WP-Scout | Fourth class **Scout** (leather, bow + blade, stealth): design done (`docs/design/scout.md`), five implementation lanes, depends on WP11's machinery. Carries the mandatory single-owner refactor of `physics_override.speed` in `grug_core` (three shipped systems depend on the current two-owner count). **Placeholder id**: the row exists so the design does not go untracked; the real WP number is the coordinator's to assign, and it deliberately avoids `WP11b`, which `skill_trees.md` §6.2 proposes for the capstone split | open (spec: `docs/design/scout.md`) | WP11 |
