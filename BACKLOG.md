...
| WP11 | Skill trees: 2 trees × 2 chains × 8 talents = 30 ranks per tree (60 per class), 1 point per 2 levels (30 at level 60 = one whole tree), 5 numeric talents + 2 keystones + 1 capstone per tree. **Per tree at most ONE keystone adds a new skill**; the other keystone and the capstone improve or REPLACE an existing button, so no build exceeds base kit + 2 hotbar keys. **5 new ability registrations** for the three shipped classes (Hold Ground, Broadstroke, Cinderfall, Glacial Ward, Word of Ruin; Renew is the sixth and already exists). Nine talents deliberately break a cap or a control rule, each with a stated cooldown/duration. Respec for money **in the talent UI, no class trainer**. **Prerequisite: the `grug_core` speed aggregator** (owned by the mob-pressure lane) for one talent | open (spec: `docs/design/skill_trees.md` PROPOSAL rev 2 + `progression.md` §2; user rulings 2026-09-16, four rounds; open decisions in `skill_trees.md` §6) | WP3, WP4 |
...
| WP-Speed | **`grug_core` speed aggregator** (~100 lines): named modifiers with independent durations, root as a hard flag, mounts outside it — replacing the two independent `physics_override.speed` owners that `grug_mobs/verbs.lua:100-118` calls out. Prerequisite for WP11's Hold Ground, for the Scout's Sprint and for the mob-pressure patch; the mob-pressure lane owns it | open (spec: `docs/research/mob-pressure-task-card.md` §4b) | — |
...
- **WP11**: the **respec price** (5c × level, min 25c) has no consumer yet —
  `grug_money.take` is the API it will call. It is the only respec number
  written down anywhere: `items_crafting.md` §8.3 has since been rewritten to
  "repeatable at the class trainer and rising with level", and the user's
  ruling of 2026-09-16 removed the class trainer entirely (respec lives in
  the talent UI). Keeping, measuring or dropping this formula is open
  decision `docs/design/skill_trees.md` §6.8 — including the option that a
  respec is free with a cooldown, which would retire the `grug_money`
  dependency edge altogether.
