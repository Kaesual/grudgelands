...
| WP40 | **Simple named-zone world foundation:** implement the 38-zone hybrid-v7 world from `docs/design/world_zones.md` §§7–14 and the current [engineering contract](docs/research/wp40-engineering-brief.md): one fixed, visually reviewed 2D layout; small authored land/water primitives; integer nearest-hub zone ownership; 57 reliable typed land routes plus separate boat links; exact Holy Grounds, four bays, nominal 80-node shelf, immutable dragon channels/deep ocean; ten housing masks; globally queryable project-owned height; logical biomes, landmarks, anchors, resources and one consolidated VoxelManip transaction. Delivery is R0–R8 in [wp40-simple-map-rebase-plan.md](docs/research/wp40-simple-map-rebase-plan.md), with a mandatory SVG visual gate after R1. Production uses one v7 emerge thread; long/full-layout and 32-seed populations use LuaJIT, targeted PUC 5.1 KATs require canonical parity. Fresh-world-only | **in progress** since 2026-08-13 on branch `wp40-named-zone-world-foundation`; simple-map rebase D1–D7, R0, R1, V1b, V1c and V1d were independently accepted 2026-08-25, and the user accepted the final V1d SVG as sufficient for the first version. T0/T1 and completed T2 artifacts remain historical evidence, but the unfinished exact partition/topology path, S1 lock/winner seeds and PCC/F1/F2 final gate are superseded and the old T2 agent will not resume. V1d provides stronger single-warp boundary meanders, pinned curved routes, tapered four-bay gaps outside capital envelopes and coastal housing cores, organic authored river/lake masks, four ownerless deep-ocean bay-mouth caps and six 128-node-wide hard-protected capital ingress corridors. R2 layout freeze/validation is next and has not started. The possible Holy Grounds rename/shared-build policy is explicitly deferred before protection/consumer integration. Later gates retain native cave/ore/dungeon/stratum preservation, the 32-seed supply/access audits, once-per-layout housing capacity and reproducible WP18-relative performance evidence | WP6, WP18, WP43 |
...
1. **WP26 — universal bars/furnace**, **WP40 — named-zone map foundation**
   ...
   independently accepted, including the final visual gate; R2 layout
   freeze/validation is next and has not started. Read
   `docs/research/wp40-simple-map-rebase-plan.md` for the current R0-R8
   sequence. Fixed-layout geometry/housing validation and once-per-layout
   capacity simulation, 32-seed content/supply audits and reproducible
   performance measurements remain mandatory outputs.
