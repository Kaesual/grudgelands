...
| WP33 | Gathering and cultural surface resources: register the six herb/spice nodes, cooking ingredients and found-only food sources, plus ordinary signature woods and other gatherable culture materials assigned by the final race-region/zone palettes. Enforce Alchemist-only healing-herb harvest, universal spices and equivalent faction access; no housing-exclusive resource exists. Register every cultural source against WP40 R6's frozen invisible 5×5×9 slot envelope before R7 activates the writer; registrations never move a slot or add a healing writer | open; exact placement uses the accepted WP40 R6 slot API and WP43 material ids; its reviewed contract decides whether a feature may replace P7 output in the reservation's lower two levels | WP40 R6, WP43 |
...
| WP40 | **Simple named-zone world foundation:** implement the 38-zone hybrid-v7 world from `docs/design/world_zones.md` §§7–14 and the current [engineering contract](docs/research/wp40-engineering-brief.md): one fixed, visually reviewed 2D layout; small authored land/water primitives; integer nearest-hub zone ownership; 57 reliable typed land routes plus separate boat links; exact shared mutable Battlegrounds, four bays, nominal 80-node shelf, immutable dragon channels/deep ocean; ten housing masks; globally queryable project-owned height; logical biomes, landmarks, anchors, resources and one consolidated VoxelManip transaction. Delivery is R0–R8 in [wp40-simple-map-rebase-plan.md](docs/research/wp40-simple-map-rebase-plan.md), with a mandatory SVG visual gate after R1. Production uses one v7 emerge thread; long/full-layout and 32-seed populations use LuaJIT, and the final representative PUC 5.1/LuaJIT micro-KAT pair requires byte-identical canonical parity. Fresh-world-only | **in progress** since 2026-08-13 on branch `wp40-named-zone-world-foundation`; simple-map rebase D1–D7 and R0–R5 are independently accepted, with fixed-layout V1e R2 visually accepted as the live horizontal authority, the pure R3 vertical implementation plus canonical artifact accepted, the first complete still-disabled R4 geography/policy payload accepted 2026-08-27, and the pure typed still-disabled R5 planner/adapter payload plus canonical artifact accepted 2026-08-29. T0/T1, completed T2 and V1d remain historical evidence, while the unfinished exact partition/topology path, S1 lock/winner seeds and PCC/F1/F2 final gate are superseded. V1e retains the accepted single-warp boundary meanders, pinned curved routes, tapered four-bay gaps outside capital envelopes and coastal housing cores, organic authored river/lake masks, four ownerless deep-ocean bay-mouth caps and six 128-node-wide hard-protected capital ingress corridors. Its exhaustive R2 evidence freezes all 100 anchors, the 74 actual POI spurs, housing capacity and three exact contact-face waterfalls without moving their reaches; R3 adds the accepted deterministic height, graded route/water operations and final immutable 3D payload; R4 adds the accepted 38-zone payload, scalar policy, logical-biome selection and sparse indexes; R5 adds the accepted pure typed Planner, consolidated native-preserving Adapter and disabled construction seam. None enables a Luanti writer yet. The 2026-08-27 Battlegrounds ruling retains internal `holy_grounds` geometry/policy ids but makes ordinary terrain editable by both factions at every depth, claim-excluded and flyable while preserving all explicit hard protection. All ten R6 design choices were approved on 2026-08-29 and are folded into `docs/design/`; exact successor-schema and arithmetic contract preparation is next. No R6 implementation starts before that contract's dedicated hard-lens and mandatory independent review pass. WP33 registrations consume the accepted R6 slot API before R7 may activate the writer. Later gates retain the 32-seed supply/access audits, once-per-layout housing capacity and reproducible WP18-relative performance evidence | WP6, WP18, WP43 |
...
1. **WP26 — universal bars/furnace**, **WP40 — named-zone map foundation**
   ...
   complete still-disabled R4 geography/policy payload and artifact and the
   pure typed still-disabled R5 planner/adapter payload and artifact are also
   independently accepted. All ten R6 design choices were approved on
   2026-08-29; the reviewed exact successor-schema/arithmetic contract is next,
   followed by R6 implementation while the writer remains disabled. Read
   ...
   once-per-layout capacity evidence; 32-seed content/supply audits and later
   production mapchunk performance measurements remain mandatory outputs.
