...
| WP31 | Mounts: implement the four decided level-15/30/45/60 riding tiers, owner-bound item/ephemeral-entity lifecycle, damage dismount, Holy-Grounds flight, enemy-territory land-only restriction and WP40 ocean/channel no-flight masks. WP44 converts the fixed 15m/45m/2h/5h reliable-net-income targets into prices. Do not invent the still-open asset, entity-damage, mounting-in-combat, underground-flight, ceiling/drift, swimmer-exhaustion, variant or trainer details | open; blocked on `TODO-design-crafting-rework.md` D12/D14–D20 | WP28, WP40, WP44 |
...
| WP40 | **Named-zone world foundation:** implement the 38-zone hybrid-v7 map from `docs/design/world_zones.md` §§7–14: stable zone/race-region/territory/PvP fields; exact adjacency and separate boat graph; independently authored mainland silhouettes; Holy Grounds; final starts/capitals/roads/water; ten housing masks; shallow hard-protection versus mutable claim-excluded envelopes; 80-node shelf, full-depth deep ocean, immutable dragon channels and both offshore island envelopes; logical biomes, level fields, anchors and resource/supply geometry. The six-part engineering brief required by `docs/research/mapgen-control.md` was written and reviewed before implementation began; it lives in [wp40-engineering-brief.md](docs/research/wp40-engineering-brief.md) (analytic chunk-order-independent anchor heights; rewrite shell/cut-fill and intersected caves/dungeons/liquids/strata; native/authored decorations and water normalization; generic versus cultural resource placement; registry initialization/memory strategy; benchmark corpus/thresholds). Production uses one v7 emerge thread. Run and record the fixed 32-seed geometry/topology/route/housing/supply audit, capacity simulation and reproducible WP18/WP36-relative performance measurements before integration. Fresh-world-only | **in progress** since 2026-08-13, on branch `wp40-named-zone-world-foundation`. The engineering brief is written and reviewed ([wp40-engineering-brief.md](docs/research/wp40-engineering-brief.md)); it decomposes the work into T0-T9. T0 and T1 are complete; T2's census is measured and closed (run 6 over all 4,123 seeds, `census-*-v3.tsv` published 2026-08-19; R20/R21 zero; all four winner seeds clean on every tier) and its findings opened the **bay-transition-simplicity correction package** (wp40-t2-contracts.md section 10 — a solo-scale debug plan with a deterministic 7-seed reproducer; scheduled within WP40 ahead of world integration); T3-T9 have not started. The T2c-E0-C1 selected-four conformance chain was migrated onto the committed v3 scalar pool (`19fc28d1`) and its separately authorized acceptance run completed green on 2026-08-22 from commit `5d770365` ([wp40-t2-contracts.md](docs/research/wp40-t2-contracts.md) section 12): 20/20 PUC rescores, all four selected slots, final artifact SHA-256 `7ac6b7f9…`, and a same-HEAD resume that re-verified all 24 retained rows without recomputation. Section 12.4 is now the frozen downstream boundary-topology interface; the final artifact still records `stage2=pending_seed_corpus_promotion`, so this is not T2-final or corpus promotion. The two slot findings that blocked it are the **section 8.6 ledger items**, now closed (section 12.5): slot 29 was the R18-level C2 conformance oracle brought forward to accepted R19 — production was right and no production geometry moved — and slot 30 was measured pin drift, a Starbough digest literal transcribed one character short. **Neither is a new topology failure and neither needed a new section 11.6 ruling**; the section 11.11 boundary-topology freeze is untouched. The same branch also makes a finished conformance artifact reusable at its recorded commit while its pinned input/code closure — the v3 authority roster — is byte-identical (section 12.6), so an unrelated commit no longer buys a 24-row rerun. `edge_inventory_sha256` stays diagnostic-only and is not added to the frozen C1 interface. The acceptance conformance is closed; seed-corpus promotion, the light-regime fields and remaining compiled buckets, selectors, production compiler, and T2-final remain open. Current state, ordering and open items: [wp40-t2-plan.md](docs/research/wp40-t2-plan.md) - read that before planning anything here. The 32-seed audits, capacity simulation and performance measurements remain mandatory outputs | WP6, WP18, WP43 |
...
1. **WP26 — universal bars/furnace**, **WP40 — named-zone map foundation**
   ...
   roots. WP40 is in progress: its engineering brief is written and reviewed, and
   T0/T1 are complete with T2 in flight. Read
   `docs/research/wp40-t2-plan.md` before planning WP40 work - it carries
   the current state, the execution order and the locked surfaces. The fixed
   32-seed geometry/supply/housing audits, capacity simulation and
   reproducible performance measurements remain mandatory outputs.
