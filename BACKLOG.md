...
| WP10 | Professions: six primaries plus secondary Cooking; one complete T1–T6 UI recipe book per profession, with crafting permission advanced by current-tier crafts and capped by the character's ten-level band. Recut Goldsmith around Quartz, Gold, the six G1/G2 gems, Rough→Cut processing, the fixed Setting ladder and both trinket slots; implement universal/cultural/PvP-special recipe ownership and direct cultural-master services. **R22:** the crafting inventory has the two primary slots, Cooking and a General book derived from global recipes, while the furnace, dual furnace and VoxeLibre-style brewing stand each have a station-filtered book. No book item, keystone unlock, Gem Detector, Amplifier or Dowsing Rod exists. Retain Alchemist herbs/draughts, Leatherworker, Tailor, Woodcarver and Blacksmith scopes without parallel gear catalogs | in progress (2026-09-18): `grug_jobs` ships the validated registry/station seam, persistent two-primary-plus-secondary state, T1–T6 advancement, grid and station extraction gates, shared UI books and all start/capital trainers. Cooking v1 now adds 15 plant items, 18 three-role dishes, six raw assembled-dish routes, three basic furnace refinements and zone-level fish tables on that API. Round 8 Alchemy still adds its catalog and brewing stand; Round 9 adds the six primary catalogs. The primary content, refinement and cultural-service remainder stays open; specs: `professions.md`, `items_crafting.md`, `inventory_equipment.md` | WP26 ✅, WP33 ✅, WP43 ✅, WP44 |
...
| WP31 | Mounts: implement the four decided level-15/30/45/60 riding tiers, owner-bound item/ephemeral-entity lifecycle, damage dismount, Battlegrounds flight, enemy-territory land-only restriction and WP40 ocean/channel no-flight masks. WP44 converts the fixed 15m/45m/2h/5h reliable-net-income targets into prices. Do not invent the still-open asset, entity-damage, mounting-in-combat, underground-flight, ceiling/drift, swimmer-exhaustion or variant details; riding itself is sold by the capital job trainers (D20 decided 2026-08-13, `mounts.md` §1) | open; blocked on `TODO-design-crafting-rework.md` D12/D14–D19 | WP28, WP40, WP44 |
...
report is evidence only: every candidate still awaits the user's individual
import decision. WP11 X3 remains open.
...
1. **WP13 — world structures**, selected by the user after WP40 completion.
   The dwarf start in Hearthpine Vale is being polished after its first accepted playtest,
   an inhabited craft settlement with a small guardpost. Its nine-building
   [second increment](docs/research/wp13-hearthpine-polish.md) has green native
   and interpreter checks plus clean review; the next GUI test is pending. The
   [bounded brief](docs/research/wp13-hearthpine-brief.md) follows the chosen
   [appearance](docs/design/settlements.md); a focused user playtest precedes
   broader rollout. The remaining six-race structures/civic scope stays open.
...
- **WP31** additionally retains the explicit mount questions in
  `TODO-design-crafting-rework.md` D12/D14–D19 (D20 was decided 2026-08-13:
  riding is a role on the capital job trainers, `mounts.md` §1). WP44 measures
  its four already-decided price targets; it does not decide those remaining
  behavior and asset questions.
