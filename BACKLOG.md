...
| WP13 | Starter/world structures: six 128×128 outer race settlements; six central capitals with 512×512 build envelopes, 704×704 terrain blends, fixed civic cores/four road gates and four seed-permuted district roles; two villages, four ordinary outposts, two bandit camps and one peaceful mining camp per race; two all-six-gem apex camp structures; level-60 capital defenders and one invulnerable MVP king each. Mining camps are the only renewable-resource structure (`world.md` §2 R4); WP34 owns their node/refill mechanic | open; complete placement/footprint contract in `world_zones.md` §§8/11/12, waits for WP40 anchors | WP40 |
...
| WP23 | Apex world bosses: two equally reachable shared-front dragons, one at The Wyrmglass Crown and one at Stormscale Summit, through one stationary-arena/telegraph/hoard/respawn chassis with two regional variants; later regional apex kits follow | open; endpoint and access contract fixed in `world_zones.md` §§6/8/9 | WP6, WP40, WP41 |
...
| WP31 | Mounts: the four riding tiers as a **universal skill** (no main-profession slot) bought at a trainer for 1s / 8s / 30s / 60s ≈ 1g — Apprentice slow land, Journeyman fast land, Expert slow flying, Master fast flying, +50 % / +100 % over the engine's 4 nodes/s walk. A mount is an **entity the player is attached to** (`mobs.attach` / `detach` / `drive` / `fly` from the already-vendored MIT `mobs_redo/mount.lua`, which needs `player_api` — we have it), speed is the **entity's velocity and never `physics_override.speed`**, and a mount carries no level, XP or threat. Open-sea "Exhausted", the no-mount housing-isle rule, enemy-territory flight ban and damage dismount remain decided. WP40 must re-derive their boundaries against authored land connections and named contested zones; no mount rule may retain the retired 200-node-strait/war-coast coordinates | open (spec: `docs/design/mounts.md`; blocked on WP40, the `open_sea_at` fix and the remaining `TODO-design-crafting-rework.md` D12–D20 questions) | WP24, WP28, WP40 |
...
| WP40 | **Named-zone world foundation:** replace WP18's radial `core/inner/outer/coast/war_coast` surface and mandatory water strait with `world_zones.md`'s complete 38-zone hybrid-v7 macro-map: stable ids and separate race-region/territory/PvP fields, spatial lookup, exact adjacency graph, bounded boundary variation, three contact loops, logical weighted biome palettes and surface dressing, outer spawn/respawn settlements, six directional L20–30 capital profiles, named-zone surface levels with the existing depth floor, shared-front resource-only digging, and migration of 24 outposts/12 camps/rares/patrols/spawns plus every downstream anchor. Fresh-world-only; pass §14's 32-seed geography, palette, level, territory, travel, content and economy audits—including both factions mining all 24 protected apex gem slots—before deleting old fields | open; **design-ready** (`world_zones.md` §§7–14) | WP6, WP18 |
...
- **WP31 (mounts) ← WP24, WP28, WP40 and the still-open half of D12–D20.**
  ...
  grace as §4.1. WP40 replaces the 200-node strait/war-coast geometry, so
  the rule must be re-verified against the authored land connections. Still
