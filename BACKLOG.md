...
| WP39 | **Crosshair-authoritative combat** — replace implicit enemy-lock damage authority with current server-ray aim while preserving WP38's runtime-proven fast held animation and one-full-swing-per-weapon-interval cadence; add a binary weapon-ready crosshair ring, permanent opt-in combat diagnostics, current-ray hostile casts, and a reusable swept-projectile foundation whose first consumer is a straight non-homing Fireball. Full task and acceptance contract below; specs: `docs/design/classes.md` §2b and `docs/design/combat_stats.md` §2. | **open — NEXT by owner direction (decided 2026-08-10, no design blocker)** | WP38 |
...
target acquisition, but their mirrored capabilities
...
1. **T0 — permanent diagnostics first.** Add server/admin-only, per-player
   `/combatdebug on|off`, cleared on leave. Disabled sites do only an enabled
   table check: no extra ray, globalstep, formatting, particles or log call.
   Enabled output is rate-limited and reports swing input/readiness/ray blocker/
   target/range/outcome/proc settlement plus cast and projectile lifecycle.
2. **T1 — one combat-ray contract.** Introduce a server helper that starts at
   the actual eye, follows current look direction to the caller's range, respects
   blocking nodes and entity selection boxes, skips self, and returns enough
   structured information for debug without running the ray twice. Melee,
   current-target hostile casts and their tests share it where their differing
   collision rules allow. Enemy target memory becomes Target-Frame/UI state
   only; ally heal/shield memory remains an implicit target.
3. **T2 — ready-until-aimed melee.** Preserve one full slot-fed swing and no
   partial ability damage. When `next_due` expires, a held swing ray with no
   valid hostile leaves the clock ready. The first valid hostile on a later
   pass consumes the interval before the punch and uses the existing exact
   claim-once/two-phase mob+PvP path. Dodge/immunity/PvP refusal/absorb/callback
   cancel consumes cadence but pays no rage/cost/charge/effect. Hold and click
   spam remain DPS-identical; skill, tool/fist, weapon-swap and lifecycle clock
   boundaries retain the anti-double-stream invariants.
4. **T3 — binary weapon-ready reticle.** Overlay a small gold ring on the stock
   crosshair only for a selected swing whose weapon clock is ready. Hide it on
   a valid attempted swing; show it once on expiry; leave it visible on an aim
   miss; remove it for non-swing selection and lifecycle cleanup. It is not a
   target-validity indicator or progress animation and never writes inventory.
5. **T4 — hostile cast authority.** Charge, Taunt and Smite require a current
   valid pointed/server-validated hostile within their existing range and LOS;
   no target means no spend/cooldown. Remove every hostile `enemy_target`
   fallback while preserving ally-memory/self behavior for heals and shields.
6. **T5 — reusable projectile mod and Fireball.** Add a dependency-clean
   `grug_projectiles` API/entity foundation with shooter ownership, faction/
   validity filtering, swept previous→new-position collision, exactly-once hit,
   node collision, max-distance/lifetime and unload cleanup. Fireball snapshots
   eye/look at cast, spends 8 mana even into empty space, travels straight at
   20 m/s for at most 20 m, has no homing/gravity/splash, ignores allies and
   dropped items, and applies the existing `6 + spell power` once to the first
   attackable target. Do not build bows; leave an API that can later add arrow
   gravity and draw-derived initial impulse without target lock.
7. **T6 — integration and regression closure.** Keep Swing loot pickup working
   with Strike/Hamstring, node pointabilities blocking post-kill digging, cast
   loot pickup not casting, Fireball mana refusal, proc/rage/threat/crit/dodge/
   absorb/lethal ordering, and ordinary target-keyed tool/fist remainder. A
   target switch forfeits damage remainder below 1 and pending rage credit;
   target memory is not used for accumulation.
8. **T7 — verification and docs.** Build a plain-Lua-5.1 real-code mock for
   ray hit/miss/block/range, ready retention and cadence consumption, click vs.
   hold, target switching, PvE/PvP cancellation, reticle transitions, hostile
   casts, ally fallback, Fireball wall/entity/ally/range/empty/mana cases and
   exactly-once projectile settlement. Run the workflow's Lua-5.1, five-sweep,
   strict-global, diff, vendored-marker, submodule and 100-player performance
   gates; synchronize BACKLOG/ROADMAP/README/AGENTS only after review is clean.

**Acceptance invariants:** aiming away while holding Strike/Hamstring produces
...
and spends mana on a miss. Disabled combat debug creates no diagnostic work;
the reticle causes state-change HUD packets only and zero inventory churn.

**Required GUI runtime test (user, after clean merge candidate):** enable
...
**WP39 is the next package by owner direction.** Its design is fully folded
into `classes.md` §2b and `combat_stats.md` §2, and its task/acceptance/runtime
contract is above. It deliberately supersedes only WP38's enemy soft-lock
target authority: the tested held animation, full-swing weapon clock,
claim-once settlement and loot bridge remain the base to preserve. There is no
blocking `TODO-*.md`; arrows themselves remain later scope.

**Ready now** (no design blockers, deps done): **WP39** (crosshair-authoritative
combat — explicitly next), **WP34** (depth economy —
