...
| WP40 | **Simple named-zone world foundation:** implement the 38-zone hybrid-v7 world from `docs/design/world_zones.md` §§7–14 and the current [engineering contract](docs/research/wp40-engineering-brief.md): one fixed, visually reviewed 2D layout; small authored land/water primitives; integer nearest-hub zone ownership; 57 reliable typed land routes plus separate boat links; exact Holy Grounds, four bays, nominal 80-node shelf, immutable dragon channels/deep ocean; ten housing masks; globally queryable project-owned height; logical biomes, landmarks, anchors, resources and one consolidated VoxelManip transaction. Delivery is R0–R8 in [wp40-simple-map-rebase-plan.md](docs/research/wp40-simple-map-rebase-plan.md), with a mandatory SVG visual gate after R1. Production uses one v7 emerge thread; long/full-layout and 32-seed populations use LuaJIT, targeted PUC 5.1 KATs require canonical parity. Fresh-world-only | **in progress** since 2026-08-13 on branch `wp40-named-zone-world-foundation`; simple-map rebase D1–D7, R0, R1, V1b, V1c and the V1d technical package were independently accepted 2026-08-25. T0/T1 and completed T2 artifacts remain historical evidence, but the unfinished exact partition/topology path, S1 lock/winner seeds and PCC/F1/F2 final gate are superseded and the old T2 agent will not resume. Initial V1 accepted the macro layout; V1d now provides stronger single-warp boundary meanders, pinned curved routes, tapered four-bay gaps that stay outside capital envelopes and coastal housing cores, organic authored river/lake masks and six 128-node-wide hard-protected capital ingress corridors. User visual approval of the generated V1d SVG is the next gate, and R2 must not freeze or repair the layout before it. The possible Holy Grounds rename/shared-build policy is explicitly deferred before protection/consumer integration. Later gates retain native cave/ore/dungeon/stratum preservation, the 32-seed supply/access audits, once-per-layout housing capacity and reproducible WP18-relative performance evidence | WP6, WP18, WP43 |
...
1. **WP26 — universal bars/furnace**, **WP40 — named-zone map foundation**
   ...
   replaced the former exact-T2 authority. R1, V1b, V1c and the V1d technical
   package are independently accepted; user visual approval of the V1d SVG is
   next. Read
