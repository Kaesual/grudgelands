...
| WP9 | Mandatory questlines: named-zone progression, first PvP quests through Ashenward March / Bannerbreak Mesa and The Broken Causeway, war-front guards, elite quests and level gates | open; geography and quest-interaction slots are fixed in `world_zones.md` §§8/16 | WP6, WP8, WP40, WP41 |
...
| WP13 | Starter/world structures: six 128×128 outer race settlements; six central capitals with 512×512 build envelopes, 704×704 terrain blends, fixed civic cores/four road gates and four seed-permuted district roles; two villages, four ordinary outposts, two bandit camps and one peaceful mining camp per race; two all-six-gem apex camp structures; level-60 capital defenders plus one killable level-65 elite king and four level-60 elite royal guards per capital. The king is authoritative for guard follow/teleport and group reset; death despawns surviving guards and the full group returns after a persistent 15 minutes. King rewards use personal participation (60-node living radius, 60-second combat-death grace, reset cleanup) and a separate rolling 24-hour Crown lockout per character and king (`world_zones.md` §12). Separate essential service NPCs remain passive and invulnerable. Mining camps are the only renewable-resource structure (`world.md` §2 R4); WP34 owns their node/refill mechanic | open; complete placement/footprint contract in `world_zones.md` §§8/11/12, waits for WP40 anchors | WP40 |
...
| WP17 | Travel: waypoint nodes at race starts/capitals/authored zone hubs, visit-unlock, travel formspec (map UI docks on with WP12), race-capital Home Stone + /unstuck | open (spec: `docs/design/world.md` §6) | WP40 |
...
| WP23 | Apex world bosses: two equally reachable offshore dragons, one on The Wyrmglass Crown and one on Stormscale Summit, through one stationary-arena/telegraph/hoard/respawn chassis with two regional variants; personal eligibility uses the 60-node living radius, 60-second combat-death grace and reset cleanup, and each dragon has an independent rolling 24-hour boss-loot lockout per character. Respawn uses a persistent 30-minute base timer plus a full unload-safe 60-second lair warning and grants no temporary invulnerability; the islands remain contested in every boss state. Later regional apex kits follow | open; island role/no-land adjacency fixed in `world_zones.md` §§6/8/9, exact channel and boat geometry waits for WP40 | WP6, WP40, WP41 |
...
| WP31 | Mounts: universal riding purchases at exact levels **15/30/45/60** — Apprentice land +50% (6 nodes/s), Journeyman land +100% (8), Expert flight +75% (7), Master flight +150% (10); exact Gold prices derive from **15m/45m/2h/5h reliable net solo-income** targets. Each tier is a persistent owner-bound inventory/hotbar item that remains in the inventory while it summons one ephemeral entity at the player's exact position; the attached player controls only that entity, and every dismount removes it. Damage always dismounts. Both factions may fly over Holy Grounds; every ocean column is non-flyable at every y, with an exact 48-node shore warning followed by hard dismount. The ≥200-node dragon channels retain ≥104 hard no-flight nodes after both warnings and keep both islands boat-only; enemy territory permits only the two land tiers. Adapt the already-vendored MIT `mobs_redo/mount.lua` attachment/drive seam without parked or consumed mounts. WP40 owns the boundary/channel lookup and the high-altitude no-island-access proof; no mount rule may retain retired rectangular strait/isle coordinates | open (spec: `docs/design/mounts.md`; blocked on WP40 and remaining `TODO-design-crafting-rework.md` D12 assets, D14–D20) | WP28, WP40 |
| WP32 | **Farming — explicitly post-MVP** (Phase 2, its own package): a Minecraft-like farming layer, adapted rather than invented — both VoxeLibre and Lord of the Test ship a working one, and the plant selection is large enough that content is never the constraint. It must fit the herb/spice split of `biomes_mobs.md` §2 exactly: **spices are farmable, healing herbs never are**; the **found-only** cooking ingredients (mushrooms, wild cocoa and rock salt) never become crops either. Farming happens only inside the player's own active open-world housing claim, and only cooking ingredients grow as crops there: the `[food]` and `[spice Tn]` lines of `biomes_mobs.md` §2. Ordinary saplings, bamboo and decorative/mundane useful plants remain allowed under the claim growth rule; growth may cross the active boundary and the resulting nodes are protected only where they lie inside it. **Mandatory asset gate:** enumerate every registered crop, sapling, bamboo and other growable variant from its real growth source and prove that no single growth transition can extend more than 10 horizontal nodes from its origin; the automated audit fails the WP if any variant exceeds that bound. The package ships the crop layer, claim restriction and this exhaustive growth audit together | open, **Phase 2** (spec: `TODO-design-housing.md` §§6.3/6.6, `biomes_mobs.md` §2/§6, `professions.md` §1; waits for housing integration) | WP10, WP24, WP33 |
| WP33 | Herb & food nodes (the MVP gathering set): register the six herbs — healing gravemoss/dragonweed/crimson lotus, Alchemist-only; spices sunleaf/marshbloom/stormkelp, universal — plus the MVP cooking ingredients and found-only mushrooms/wild cocoa/rock salt. Placement follows the fixed biome palettes of the named-zone catalog, with equivalent tier access on both faction sides; punching a healing herb without the Alchemist yields nothing plus a hint | open (spec: `biomes_mobs.md` §2/§6, `professions.md` §1) | WP40 |
...
| WP40 | **Named-zone world foundation:** replace WP18's radial `core/inner/outer/coast/war_coast` surface and mandatory water strait with `world_zones.md`'s 38-zone hybrid-v7 macro-map: stable ids and separate race-region/territory/PvP fields, spatial lookup, exact land-adjacency plus separate boat graph, bounded shared-edge variation, independently authored three-prong/capital-belt/front silhouettes for both continents, exact seven-cell capital belts at |z| = 1100..1900 and fixed three-sector frontier control lines through |z| = 900 to the Holy Grounds, analytic planned-mainland footprints with exact independently authored outer-perimeter chains whose four fixed variable-width bay masks and other internal lakes/rivers remain zone water and whose exterior perimeter starts the 80-node shelf/deep-ocean sequence, one universal generated surface-water node with biome/landmark-derived bed and shore dressing, the exact x = −2500..+2500 / z = −250..+250 four-zone Holy Grounds chain, fixed dragon-island centres/envelopes, ≥200-node immutable channels with 48-node warning bands, ≥104-node hard-flight strips and paired 96-node boat approaches at z = ±125, six fixed start anchors with centred 600×500 dry cores and exact clipped start/home control polylines creating one-neighbor start caps and flank-wrapping home necks into exact capital-cell intervals, four guaranteed dry-land coastal housing strips each retaining ≥600 continuous shoreline nodes, ≥300 buildable inland nodes and ≤12 natural-ground relief in every core-contained 101×101 footprint after variation/exclusions, deterministic primary 7/16, secondary 5/12 and trail 3/8 road/exclusion classes assigned to every §9.4 graph edge including six complete Holy-Grounds crossings, six water-level-relative macro-relief profiles with explicit secondary/landmark masks and deterministic grading priority, logical weighted biome palettes and authored surface dressing, outer spawn/respawn settlements, six directional L20–30 capital profiles, named-zone surface levels with the depth floor, universal contested terrain rights from level 31 outside explicit exceptions, and migration of 24 outposts/12 camps/rares/patrols/spawns plus every downstream anchor. Before code changes, its orchestrator must write and review the engineering brief required by `mapgen-control.md`: analytic terrain-derived anchor heights, rewrite shell/cut-fill and intersected-feature policy, native/authored decoration and resource split, registry initialization strategy, and benchmark corpus plus thresholds. Implement the single authoritative mapgen-environment overlay researched there; fresh-world-only. Pass §14's 32-seed geometry, topology, relief, palette, level, territory, travel, housing-capacity, content and economy audits—including all 24 protected apex gem slots—and record capacity/performance measurements before deleting old fields | open; **design-ready**: complete macro geometry, anchors, universal-water/dressing rule, road graph, housing envelopes, all 38 relief/landmark packages and acceptance gates are fixed (`world_zones.md` §§7–14, `TODO-design-housing.md`) | WP6, WP18 |
| WP41 | **PvP tag and geographic eligibility:** implement `world_zones.md` §15's single `grug_pvp` seam across ordinary melee, authoritative swings, casts, AoE, projectiles, support and protected faction NPCs; safe→safe tags attacker but blocks the first effect, safe→tagged lands after tagging, tagged→safe blocks, effective HP/absorb/support refreshes 60 s, full contested tail, death clear, persisted reconnect, per-target boundary snapshots, enemy-service refusal and sword/shield HUD/Target-Frame states | open; **design-ready**, implementation waits for WP40's zone API | WP39, WP40 |
| WP42 | **War-front life:** 16 clash anchors across eight designated war-activity zones within the twelve contested zones; at most one four-vs-four encounter per such zone (64 dedicated NPC global worst case), deterministic 8–14 min windows, 128-node activation, 160-node/90 s withdrawal, no catch-up/refill, opposing-war-unit/eligible-player/reactive-creature targeting only, existing player-involvement trophy rules, no supply crates and no territory capture. Ordinary guards retain `attack_npcs = false`; WP13 owns structure art | open; **design-ready**, implementation waits for WP40/WP41 | WP6, WP40, WP41 |
...
§4 and never rolled out). The jungle-fringe ground question was closed by the
named-zone pass on 2026-08-11: WP40 pairs it with deep-jungle canopy litter.
WP36 is **not runtime tested** and its mapgen changes need a fresh world.
...
the PvP punch was ungated and stacked with the skill, and the shared swing
clock was winner-takes-all). WP38 first re-scoped those defects into the proc
model, then its runtime correction restored one authoritative full-swing held
clock while keeping ordinary melee proportional; WP39 has since replaced that
clock's enemy-lock aim authority. WP35 also left the two-handed rule
**dormant** until WP14 and handed the weapon-durability question to WP22.
It is **not runtime tested**.

**WP39 ✅ shipped 2026-08-10.** Its current-ray target authority, swept
projectile foundation and Fireball are folded into `classes.md` §2b and
`combat_stats.md` §2; the completed acceptance contract and outstanding
user-owned GUI runtime plan are above. Arrows themselves remain later scope.

**Ready now** (no design blockers, deps done): **WP34 is the next package**
(depth economy —
...
strata and raw items it smelts), **WP37** (the decided 0.75 surface-density
multiplier across the surface roster; WP6 shipped its only dependency),
**WP16** (guilds — WP7 ✅
...
WP7 ✅, the only new WP with nothing in front of it),
WP11 (talents — spec progression.md §2), WP14 (offhand), WP20 (party) and
WP21 (recovery/innkeeper). The named-zone decision of 2026-08-10 moved WP33,
WP13, WP17 and WP23 out of this list until WP40 fixes their geography.
...
- **WP40 is design-ready.** `world_zones.md` §§7–14 own all 38 ids,
  palettes, graph edges, geometry budgets, anchors, public APIs and the
  acceptance gate. It supersedes the shipped WP18 surface and is
  fresh-world-only
- **WP41 ← WP40.** Its complete transaction, support, timer,
  boundary/projectile, lifecycle, visitor and UI rules are
  `world_zones.md` §15
- **WP42 ← WP40/WP41.** Its 16 anchors, squads, schedule, caps, targeting,
  loot and no-capture rules are `world_zones.md` §16
- **WP13, WP17, WP12, WP23 and WP33 ← WP40** (WP23 also WP41): none may place
  permanent structures, routes, labels, dragons or gathering nodes against
  the retired ring map
- WP8 ← `progression.md` §4 (quest structure/level gates); **WP9** additionally
  waits on WP40/WP41 so its first PvP quest names a real contested zone and
  uses the central tag rule
- **WP34 piece 4** stays a generic mining-camp refill mechanism: ordinary
  camps declare regional-tier slots; the two endpoint camps declare exactly
  two of every race-gem slot. WP13 supplies those structures after WP40, and
  the parallel material review supplies the six ids; neither blocks WP34's
  dormant generic mechanism
- **WP24 (housing)** is design-unblocked since 2026-08-07 (world.md §5
  ...
  would spawn Kraken Guards on housing beaches (world.md §2b)
- **WP31 (mounts) ← WP28, WP40 and the remaining D12/D14–D20 choices.**
  `mounts.md` now fixes exact level-15/30/45/60 speed tiers, net-income price
  targets, the permanent hotbar-item/ephemeral-entity lifecycle, damage
  dismount, Holy-Grounds flight, spatial ocean warning plus hard dismount and
  the enemy-territory land-only rule. WP40 replaces every old strait/isle
  coordinate and must prove the dragon islands cannot be reached by high-flight
  dismount drift. Still open before WP31 can be cut: mount assets (D12), mount
  entity damage behavior (D14), mounting during combat (D15), underground
  flight (D16), a flight ceiling (D17), the separate swimmer-ocean rule (D18),
  skins (D19) and the trainer NPC (D20). D13 is closed: no parked entity exists.
