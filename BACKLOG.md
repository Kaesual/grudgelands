...
| WP38 | **The melee path: gate PvP punches, and make the shared swing clock fair** — two measured defects WP35 left behind, in the same piece of code, neither closable without a `grug_core` change. **(1) The held-button path is still ungated against PLAYERS.** `grug_core.accept_melee_swing` has exactly two callers — the cadence gate in `mods/ENTITIES/mobs/api.lua:2712` and the Strike's own swing — and the first is a mobs_redo `on_punch`, so it is only ever reached when the thing punched is a (grug_mobs-registered) mob. A punch on a **player** goes through `PlayerSAO::punch` → `on_punchplayer` and never touches the clock, so against another player the raw engine `tflp` path still runs **and stacks with the Strike**: measured over 30 s at `dtime 0.09`, Strike alone 180 damage, Strike plus a held dig with a fleshy-25 hotbar weapon **930 (5.17×)**; with equal weapons a clean ~2×. That is the `combat_stats.md` §2 PvP carry-over, and weapon-slot design E6's claim that the Strike closes it holds only for the damage the *Strike* deals. Closing it for real is the cadence port to player punches (`core.register_on_punchplayer` returning true to suppress the engine's damage, `doc/lua_api.md:6589`) with its own hostility/dodge/threat decisions — i.e. this WP absorbs the "PvP work package" carry-over below. **(2) The shared melee clock is winner-takes-all by requested interval.** It stores a timestamp and nothing else (`grug_core/combat.lua:521-530`); the acceptance threshold is whatever the *current* caller asks for, so whoever asks with the shorter interval stamps it often enough to starve the slower asker. Measured over 30 s at `dtime 0.09`: greataxe Strike alone 198 damage; the same greataxe plus a held **bare-hand** dig **69 (0.35×** — one Strike swing in 30 s**)**; plus a held steel pick 159 (0.80×). The design says the equipped weapon is the single source of damage (`inventory_equipment.md` §2); the clock says the fastest thing the player touched is. Introduced by WP35's own fix for the stacking exploit, so this is a refinement of that fix and **not** a revert — the clock has to remember the accepted swing's own interval alongside its time and refuse on `max(requested, accepted)`, so the slot weapon's cadence governs and the carry-over path is the one that starves. Fix (1) would inherit the defect, so (2) goes first. | open (spec: `docs/design/combat_stats.md` §2) | WP35 ✅ |
...
swing clock is winner-takes-all). It also left the two-handed rule
...
path — no design work needed at all: both halves are measured, and the
clock half is a small, self-contained change in `grug_core`),
**WP33** (herb &
